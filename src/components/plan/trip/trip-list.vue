<template>
  <div class="wrapper trip-wrpper">
    <div class="system-tab">
      <ul class="tab-box">
        <router-link :class='topMenuConst === "operator" ? "on" : ""' to="/trip" v-if="$menuPermission('api/product') && (ORG_TYPE !== '6')" tag="li">本公司行程</router-link>
        <router-link :class='topMenuConst === "operator-supplier" ? "on" : ""' to="/supplierTrip" v-if="$menuPermission('api/product/supplier_product_lists')" tag="li">供应商行程</router-link>
        <router-link :class='topMenuConst === "supplier" ? "on" : ""' to="/lineTrip" v-if="$menuPermission('api_supplier/product/lists')" tag="li">线路行程</router-link>
        <router-link v-if="$menuPermission('api/intention/get_intent_list')" to="/plan/customized-demand" tag="li">定制需求</router-link>
        <!-- <router-link v-if="$menuPermission('api/bpm/product')"  to="/group" tag="li">TDM行程</router-link> -->
      </ul>
      <div class="search-item tab-button">
        <router-link v-if='topMenuConst === "operator" && $menuPermission("api/product/do_step1")' :to="{path: 'addNewTrip'}">
          <el-button type="primary">+ 新增</el-button>
        </router-link>
        <router-link v-else-if='topMenuConst === "supplier" && $menuPermission("api_supplier/product/do_step1")' :to="{path: 'supplierAddNewTrip'}">
          <el-button type="primary">+ 新增</el-button>
        </router-link>
      </div>
    </div>
    <div class="system-search">
      ...
      <div v-if="topMenuConst === 'operator'" class="search-item long">
      ...
      <div v-if="topMenuConst === 'operator-supplier'" class="search-item long">
      ...
      <div class="content-list">
        <div class="mode-table">
          <el-table :data="res_list" class="el-table--middle" border style="width: 100%">
            ...
            <el-table-column
              :label='topMenuConst === "supplier" ? "op" : (topMenuConst === "operator" ? "供应商/本公司OP" : "供应商")'
              align="center"
              min-width="150"
              key="b">
              <template slot-scope="scope">
            ...
            <el-table-column
              v-if="topMenuConst === 'operator-supplier'"
              label='审核状态'
              align='center'
              width='80'
              key="a">
            ...
            <el-table-column
              ...
              width="130">
              <template slot-scope="scope">
                <div v-if="isEmptyArray(scope.row.channel_info)">--</div>
                <div
                  v-else
            ...
            <el-table-column
             ...
             >
              <template slot-scope="scope">
                <el-dropdown v-if="topMenuConst === 'operator-supplier'" trigger="click">
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item>
                      <router-link :to="{path: '/plan/approval-detail', query: {id: scope.row.approval_id}}">审批单</router-link>
                    </el-dropdown-item>
                    <el-dropdown-item>
                ...
                <el-dropdown v-else trigger="click">
                  ...
                  <el-dropdown-menu slot="dropdown">
                    ...
                    <el-dropdown-item>
                      <router-link v-if="$menuPermission('api/product/do_easy_step1') && topMenuConst === 'operator' && scope.row.type != 2" :to="{ path: '/addNewTrip', query: { id: scope.row.id, type: scope.row.product_type + 1 }}">编辑</router-link>
                      <router-link v-if="$menuPermission('api_supplier/product/do_step1') && topMenuConst === 'supplier'" :to="{ path: '/supplierAddNewTrip', query: { id: scope.row.id, type: scope.row.product_type + 1 }}">编辑</router-link>
                    </el-dropdown-item>
                    <el-dropdown-item>
                      <div v-if="$menuPermission('api/product/del_product') && topMenuConst === 'operator' && scope.row.type != 2" @click="handleDelete(scope.row)">删除</div>
                      <div v-if="$menuPermission('api_supplier/product/del_product') && topMenuConst === 'supplier'" @click="handleDelete(scope.row)">删除</div>
                    </el-dropdown-item>
                    <el-dropdown-item>
                      <router-link v-if="$menuPermission('api/product/do_easy_step1') && topMenuConst === 'operator'" :to="{ path: '/addNewTrip', query: { copyId: scope.row.id, type: scope.row.product_type + 1  }}">复制行程</router-link>
                      <router-link v-if="$menuPermission('api_supplier/product/do_step1') && topMenuConst === 'supplier'" :to="{ path: '/supplierAddNewTrip', query: { copyId: scope.row.id, type: scope.row.product_type + 1 }}">复制行程</router-link>
                    </el-dropdown-item>
                    <el-dropdown-item v-show="scope.row.status === 1 && $menuPermission('api/product/change_status') && topMenuConst === 'operator' && scope.row.type != 2"><div @click="changeStatus(scope.row.id, -1)">禁用</div></el-dropdown-item>
                    <el-dropdown-item v-show="scope.row.status === -1 && $menuPermission('api/product/change_status') && topMenuConst === 'operator' && scope.row.type != 2"><div @click="changeStatus(scope.row.id, 1)">启用</div></el-dropdown-item>
                    <el-dropdown-item v-show="scope.row.status === 1 && $menuPermission('api_supplier/product/change_status') && topMenuConst === 'supplier'"><div @click="changeStatus(scope.row.id, -1)">禁用</div></el-dropdown-item>
                    <el-dropdown-item v-show="scope.row.status === -1 && $menuPermission('api_supplier/product/change_status') && topMenuConst === 'supplier'"><div @click="changeStatus(scope.row.id, 1)">启用</div></el-dropdown-item>
                    <el-dropdown-item>
                      <router-link v-if="$menuPermission('api/product/do_price_type') && topMenuConst === 'operator' && scope.row.type != 2" :to="{ path: '/bids', query: { id: scope.row.id }}">团期报价</router-link>
                      <router-link v-if="$menuPermission('api_supplier/product/do_price_type') && topMenuConst === 'supplier'" :to="{ path: '/supplierBids', query: { id: scope.row.id }}">团期报价</router-link>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="$menuPermission('api/product/channel_operation') && !isEmptyArray(scope.row.channel_info)">
                      <a href="javascript:;" @click="handleProductOperation(scope, 'show')">渠道操作</a>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="$menuPermission('api/product/save_travel_detail_pdf') || $menuPermission('api/product/save_travel_detail_word') || $menuPermission('api_supplier/product/save_travel_detail_pdf') || $menuPermission('api_supplier/product/save_travel_detail_word')">
                    ...
                    <el-dropdown-item v-if="topMenuConst === 'supplier'">
                      <router-link :to="{ path: '/supplierTripDetail', query: { id: scope.row.id }}">查看行程</router-link>
                    </el-dropdown-item>
    ...
    <div class="page">
      <el-pagination  layout="prev, pager, next" :total="total_page" :current-page="res_search.page" @current-change="currentPageChange">
      </el-pagination>
    </div>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <template v-if="scope.row.type == 1">
              <router-link 
                v-if="$menuPermission('api/bpm/product')"  
                :to="{path: '/tdmDetail', query: {id: scope.row.product_id}}"
              >{{scope.row.edit_name}}</router-link>
              <a
                v-if="$menuPermission('api/product/send_back') && scope.row.submit_status == 2"
                href="javascript:;" @click="handleProductOperation(scope, 'withdraw')"
              >撤回</a>
              <a 
                v-else-if="$menuPermission('api/product/sold_out') && scope.row.submit_status == 3" 
                href="javascript:;" 
...
export default {
  ...
  methods: {
    ...
    getProduct(type) {
      ...
      if (this.$menuPermission('api_supplier/product/lists') && this.ORG_TYPE === '6' && this.topMenuConst !== "supplier") {
        this.$router.push({
          path: '/lineTrip'
        })
        return false
      ...
      } else if (this.topMenuConst === 'operator-supplier') {
