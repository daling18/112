    ...
    <div class="panel" v-if="type == 1">
      ...
      <div class="bd">
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div class="ellipsis" :title="scope.row.tuan_name" v-if="$route.meta.isSale">
                ...
                <router-link v-else :to="{path: '/sale/invoice/detail/tripDetail', query: {id: scope.row.product_id}}">{{scope.row.tuan_no}}&nbsp;&nbsp;{{scope.row.tuan_name}}</router-link>
              </div>
              <div class="ellipsis" :title="scope.row.tuan_name" v-else>
                <span v-if="scope.row.product_id == ''">{{scope.row.tuan_no}}&nbsp;&nbsp;{{scope.row.tuan_name}}</span>
                <router-link v-else :to="{path: '/tripDetail', query: {id: scope.row.product_id}}">{{scope.row.tuan_no}}&nbsp;&nbsp;{{scope.row.tuan_name}}</router-link>
              </div>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              <router-link :to="`/control/cost/detail?id=${scope.row.bills_id}`">
                {{scope.row.item_no}}
              </router-link>
            </template>
          </el-table-column>
    ...
    <div class="panel" v-if="type != 1">
      ...
      <div class="bd">
        <el-table
          ...
          >
            <template slot-scope="scope">
              <router-link :to="`/control/cost/detail?id=${scope.row.id}`">
                {{scope.row.item_no}}
              </router-link>
            </template>
          </el-table-column>
...
import {
  ...
  invoiceTodoAddCost,
  invoiceSaleAddCost,
  invoiceSaleTodoAddCost,
  invoiceAddCostSingle,
  invoiceTodoAddCostSingle,
...
export default {
  ...
  created(){
    ...
    switch(this.opreateRole){
      case 1:
        this.info.invoice_from = 1;
        break;
      case 2:
        this.info.invoice_from = 3;
        this.$route.meta.isSale = true;
        break;
    }
  },
  methods: {
    initialize() {
      let api = '',
        params = '';
      if(this.type != 1){
        api = this.opreateRole == 1 ? invoiceAddCost : invoiceSaleAddCost;
        params = {
          data: {
            id: this.id
          }
        ...
        api = this.opreateRole == 1 ? invoiceAddCostSingle : '';
        params = {
          data: {
            bills_id: this.id
          }
          ...
          if(res.data.code == 0){
            ...
            if(this.type != 1){
              ...
              if(this.id){
                ...
                // 公司账户带入值
                this.selectAccount = {
                  id: this.info.account_id, 
                  account_no: this.info.bank_number, 
                ...
                if(Array.isArray(info.bills_lists)){
                  ...
                  this.$nextTick(() => {
                    ...
                    info.bills_lists.forEach(item => {
                      // 在表格中选中勾选订单
                      if(item.invoice_flag){  // 当前临时使用invoice_flag判断，后续需调整 20180518 chenqz
              ...
              // 单项
              this.info = this.allData.invoice_info;
              ...
              if(this.id){
                ...
                this.table = [];
                this.table.push(this.allData.bills_info);
              }else{
                this.getMatchAccount('');
                this.getMatchBillsOrg('');
              }
    ...
    handleAddCost() {
      ...
      let api = '';
      if(this.type != 1){
        api = this.opreateRole == 1 ? invoiceTodoAddCost : invoiceSaleTodoAddCost;
      }else{
        api = this.opreateRole == 1 ? invoiceTodoAddCostSingle : '';
      }
      api({
        params: {data: info},
        success: (res) => {
    ...
    handleSelectBillsOrg(item){
      ...
      if(item && this.info.supplier_org_id == item.supplier_org_id){
        return false;
      }
