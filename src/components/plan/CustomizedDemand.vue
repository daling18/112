<template>
  <div class="wrapper">
    <div class="system-tab">
      <ul class="tab-box">
        <router-link to="/trip" v-if="$menuPermission('api/product') && (ORG_TYPE !== '6')" tag="li">本公司行程</router-link>
        <router-link to="/supplierTrip" v-if="$menuPermission('api/product/supplier_product_lists')" tag="li">供应商行程</router-link>
        <router-link to="/lineTrip" v-if="$menuPermission('api_supplier/product/lists')" tag="li">线路行程</router-link>
        <li class="on">定制需求</li>
      ...
      <div class="content-list">
        <div class="list">
          <el-table :data="tableData" border tooltip-effect="light">
            ...
            <el-table-column label="游客人数" align="center" width="100" prop="departure_people_num" />
            <el-table-column
            ...
            <el-table-column label="操作" width="100" align="center" fixed="right">
              <template slot-scope="scope">
                <router-link v-if="$menuPermission('api/intention/add_intention')" :to="'customized-demand/detail?id=' + scope.row.id">
                  查看
                </router-link>
                <a v-if="$menuPermission('api/intention/transfer_intent') && scope.row.is_deal_with < 2" href="javascript:;" @click="handleTransfer('show', scope.row)">指派</a>
              </template>
            </el-table-column>
          </el-table>
        </div>
...
export default {
  ...
  methods: {
    ...
    handleChangeScheduledTime(date) {
      this.searchInput.departure_date = date[0] || '';
      this.searchInput.back_date = date[1] || '';
    },
    handleChangeCreateTime(date) {
      this.searchInput.create_time = date[0] || '';
      this.searchInput.end_time = date[1] || '';
    },
    handleSearch(){
      this.getList(true);
