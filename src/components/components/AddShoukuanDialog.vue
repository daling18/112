...
<template>
  ...
  >
    ...
    >
      ...
      <el-form-item v-if="from == 1" label="应收款公司">
        {{ shoukuan.payment_name }}
      </el-form-item>
...
export default {
  ...
  methods: {
    ...
    getAccountList() {
      ...
      this.getAccountListType({
        params: obj,
        success: params => {
          let data = params.data;
          if (data.code == '0') {
            let arr = data.data.list;
            this.shoukuan.payment_name = data.data.payment_name;
    ...
    handleSubmit() {
      this.$refs['shoukuanForm'].validate(valid => {
        if (valid) {
          let account=  this.accountList.find(v=>this.shoukuan.account_id==v.id)
          this.$emit('submit', this.shoukuan,account)
        }
      })
    }
  }
