...
<script>
import {addStaff, staffGetOrg, todoAddStaff, getNextOrginations} from 'src/api/api';
import {addSelfUser, todoAddSelfUser} from 'src/api/user';
export default {
  name: 'UserGain',
  data() {
    return {
  ...
  methods: {
    ...
    submitForm(formName){
      const func =this. isSelf ? todoAddSelfUser : todoAddStaff
      let _this = this;
