          ...
          <div class="table-module">
              ...
              <table class="table-custom">
                ...
                <tr v-if="orderByData && orderByData.order_info">
                  ...
                  <td>{{orderByData.order_info.rmb_unpaid_amount}}</td>
                  <td>{{orderByData.order_info.distributor_name}}</td>
                  <td>{{orderByData.order_info.status_name}}</td>
                  <td>
                    <el-dropdown trigger="click" v-if="orderByData.order_info.allow_operation && orderByData.order_info.status_code != 'OS011'">
          ...
          <div class="table-module">
              <table class="table-custom">
                ...
                <tr v-if="orderByData" v-for="(item,idx) in orderByData.flight_detail" :key="idx">
                  ...
                  <td>{{seatTypeText(item.seat_type)}}</td>
                </tr>
                </template>
              </table>
          </div>
...
<script>
  ...
  import ChangeOrderOperator from 'src/plugins/ChangeOrderOperator.vue';
    ...
    components:{
      ...
      seatTypeText(type){
        // 舱位基本类型：1、经济舱 2、公务舱 3、头等舱 4、超级经济舱 5、超级公务舱 6、超级头等舱
        let text = '';
