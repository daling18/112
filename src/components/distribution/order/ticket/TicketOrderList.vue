<template>
  <div>
    <!-- <template v-if="getOrderOption.type == 'MyOrder'">
      <my-order-tab name="ticket"></my-order-tab>
    </template>
    <template v-if="getOrderOption.type == 'PurchaseOrder'">
      <purchase-order-tab name="ticket"></purchase-order-tab>
    </template>
    <template v-if="getOrderOption.type == 'SingleOrder'">
      <single-order-tab name="ticket"></single-order-tab>
    </template> -->
      ...
      <div class="search-line-item">
        ...
        <div class="search-item">
          ...
          <el-select v-model="filter.status_code" placeholder="请选择订单状态">
            ...
            <el-option label="退票审核中" value="21"></el-option>
          </el-select>
        </div>
        <div class="search-item">
    ...
    <el-table class="el-table--middle" :data="order_list" border style="width: 100%">
      ...
      <el-table-column label="操作"
                       ...
                       width="100" :resizable="false">
        <template slot-scope="scope">
          <router-link :to="{ path: '/distribution/ticket-order/detail',
                                  query: { order_id: scope.row.order_id }}"
                                  v-if="$route.name == 'DistributionTicketMyOrder'"
                                  class="look">查看</router-link>
          <router-link :to="{ path: '/plan/single-ticket-order/detail',
                                  query: { order_id: scope.row.order_id }}"
                                  v-if="$route.name == 'DistributionTicketSingleOrder'"
                                  class="look">查看</router-link>
          <router-link :to="{ path: '/plan/selfsupport-ticket-order/detail',
                                  query: { order_id: scope.row.order_id }}"
  ...
  computed: {
    orderType(){
      let routeName = this.$route.name,
          type = '';
      switch(routeName){
        case 'DistributionTicketMyOrder':
          type = 'MyOrder';
          break;
        case 'DistributionTicketPurchaseOrder':
          type = 'PurchaseOrder';
          break;
        case 'DistributionTicketSingleOrder':
          type = 'SingeOrder';
          break;
