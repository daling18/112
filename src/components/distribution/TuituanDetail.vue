<template>
  <div class="wrapper">
    ...
    <el-dialog title="填写退款金额" :visible.sync="lossAssessDialog.show" :close-on-click-modal="false">
      ...
      <div v-if="lossAssessDialog.option == 1">
        <div class="mtb15">
          订单应退款金额：<span class="color-orange">{{ lossAssessRemainAmount }}</span>
        </div>
        <el-table :data="lossAssessDialog.tourists" border style="width: 100%;">
          ...
          <el-table-column label="应退款金额" align="center" width="100">
            <template slot-scope="scope">
              ...
              <input-number  v-model="scope.row.refund_amount" :precision="3"/>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
export default {
  ...
  computed: {
    ...
    orderDetailUrl() {
      ...
      return urlPrefix + this.tuanId + '/' + this.orderId;
    },
    getCancelDetailFunc() {
      switch(this.topMenuConst) {
      case 'sale': 
