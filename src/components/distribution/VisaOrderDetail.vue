<template>
  <div class="details">
    ...
    <change-order-operator
      v-if="$menuPermission('api/order/set_order_operation') && isOperator"
      ref="operator"
      order-type="7"
...
export default {
  ...
  computed: {
    ...
    cancelOrderPerm() {
      ...
      case 'sale':
      case 'operator':
        return this.$menuPermission('api/selfsupport.visaorder/cancel_order');
      default:
        return false;
      }
    },
    ...
    confirmOrderPerm() {
      switch (this.topMenuConst) {
      case 'operator':
        return this.$menuPermission('api/selfsupport.visaorder/confirm_order');
      default:
        return false;
      }
    },
  ...
  methods: {
    ...
    handleOrderOperate(cmd) {
      ...
      case 2:   // '添加收款'
        ...
        break;
      case 3:   // '查看收入'
        this.$router.push({path: 'income-detail', query: { bid: this.orderDetailData.bill_id, type: this.isOperator ? 9 : 22 }});
        break;
      case 4:   // '申请退团', 
        this.handleRefund();
        break;
