      ...
      <div v-if="hotelDetail" class="hotel-content">
        <div class="hotel-info">
          ...
          <div class="hotel-inner">
            ...
            <div class="hotel-introduction">
              ...
              <p v-html="hotelDetail.hotel_intro" :style="moreHeightAuto"></p>
              <template>
                 <a href="javascript:;" @click="moreHeightAuto = 'height:auto'" v-if="moreHeightAuto =='height:160px'"> 查看更多>></a>
                 <a href="javascript:;"  @click="moreHeightAuto = 'height:160px'" v-if="moreHeightAuto =='height:auto'"> 收起>></a>
              </template>
            </div>
          </div>
        </div>
        <div class="hotel-album">
          <ul :style="{ width: (hotelAlbum.length * 190) + 'px' }">
            <li v-for="(img, key) in hotelAlbum" :key="key">
              <img :src="img">
            </li>
          </ul>
        </div>
      </div>
      <div v-if="$menuPermission('api/hotel/price')" class="hotel-rooms">
              ...
              <div class="rooms-content">
                <div v-for="(price, keys) in room.price_info" :key="keys" class="rooms-info">
                  ...
                  <div class="rooms-operate">
                    <!-- <el-button v-if="$menuPermission('api/hotel/createOrder')" type="primary" @click="$router.push({ name: 'DistributionHotelCheckout', query: { source: price.source, hotelId: $route.query.hotelId, rateCode: price.rate_code, time: +new Date(filter.time[0]) + '|' + +new Date(filter.time[1]) } })">预订</el-button> -->
                    <el-button v-if="$menuPermission('api/hotel/createOrder')" type="primary" @click="$router.push({ name: 'DistributionHotelCheckout', query: { source: price.source, hotelId: price.hotel_code, rateCode: price.rate_code, time: +new Date(filter.time[0]) + '|' + +new Date(filter.time[1]) } })">预订</el-button>
                  </div>
                </div>
              </div>
            </div>
