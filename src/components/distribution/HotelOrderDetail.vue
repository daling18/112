  ...
  <div class="el-tabs el-tabs--border-card">
    <div class="el-tabs__header">
        <div class="el-tabs__nav-wrap">
            <div class="el-tabs__nav">
                ...
                <div v-if="orderDetail && orderDetail.order_info.refund_status" class="el-tabs__item" @click="$router.push({ name: orderTypeRouterName.orderRefundDetailName, query: { orderId: $route.query.orderId }})">
                    退款详情
                </div>
            </div>
        </div>
        ...
        <div v-if="orderDetail" class="hotel-detail">
          <div class="panel">
            ...
            <div class="bd">
              ...
              <p class="mb10" v-if="(orderType == 'MyOrder' || orderType == 'SingleOrder') && orderDetail.order_info.source != 4">自营订单：{{orderDetail.order_info.other_order_id}}</p>
              ...
              <table class="table-order">
                ...
                <tbody>
                  <tr>
                    ...
                    <td>
                      <el-dropdown v-if="orderDetail.order_info.order_status != 5 && orderDetail.order_info.order_status != 4">
                        <span class="el-dropdown-link">
                          操作<i class="el-icon-caret-bottom el-icon--right"></i>
                        </span>
                        <el-dropdown-menu slot="dropdown">
                          <template v-if="!orderDetail.order_info.is_self_order && orderDetail.order_info.order_status == 1">
                            <el-dropdown-item v-if="$menuPermission('api/hotel/cancelOrder')">
                          ...
                          <template v-if="$menuPermission('api/hotel/orderRefund') && !orderDetail.order_info.is_self_order && (orderDetail.order_info.order_status == 2 || orderDetail.order_info.order_status == 3)">
                            <el-dropdown-item>
                              <a href="javascript:;" @click="handleRefundOrder">申请退款</a>
                            </el-dropdown-item>
                          </template>
                          <template v-if="$menuPermission('api/hotel/confirmOrder') && orderDetail.order_info.is_self_order && orderDetail.order_info.order_status == 2 && orderDetail.order_info.change_op_flag">
                            <el-dropdown-item>
                          ...
                          <template v-if="orderDetail.order_info.is_self_order && 
                            ...
                            <el-dropdown-item v-if="$menuPermission('api/tuan/save_team_order_gathering')">
                          ...
                          <el-dropdown-item v-if="$menuPermission('api/order/set_order_operation') && orderType != 'MyOrder' && orderDetail.order_info.change_op_flag">
    ...
    <el-dialog
      ...
      <span slot="footer" class="dialog-footer">
        <el-button @click="handleConfirmOrder(3)">确认无效</el-button>
        <el-button type="primary" @click="handleConfirmOrder(2)">确认有效</el-button>
      </span>
    </el-dialog>
    <online-pay v-if="$menuPermission('api/pay/pay_order') && orderDetail && orderDetail.order_info.order_status == 1" ref="onlinePay" :order-id="orderDetail.order_info.order_id" order-resource="3" />
    <change-order-operator v-if="$menuPermission('api/order/set_order_operation')" ref="operator" order-type="2" @change-success="getHotelOrderDetailHandler"></change-order-operator>
  </div>
</template>
<script>
...
import { getDistributionHolelMyOrderDetailAsyncRequest, 
         ...
         getDistributionHotelOrderHasAvail } from 'src/api/hotel';
...
export default {
  ...
  computed: {
    orderType(){
      let routeName = this.$route.name,
          type = '';
      switch(routeName){
        case 'DistributionHotelMyOrderDetail':
          type = 'MyOrder';
          break;
        case 'DistributionHotelPurchaseOrderDetail':
          type = 'PurchaseOrder';
          break;
        case 'DistributionHotelSingleOrderDetail':
          type = 'SingleOrder';
          break;
        case 'DistributionHotelSelfSupportOrderDetail':
     ...
     orderTypeRouterName(){
      ...
      switch(type){
        case 'MyOrder':
          orderRefundDetailName = 'DistributionHotelMyOrderRefundDetail';
          orderIncomeDetailName = 'DistributionHotelMyOrderIncomeDetail';
          configType = 18;
          break;
        case 'PurchaseOrder':
          orderRefundDetailName = 'DistributionHotelPurchaseOrderRefundDetail';
          orderIncomeDetailName = 'DistributionHotelPurchaseOrderIncomeDetail';
          configType = 18;
          break;
        case 'SingleOrder':
          orderRefundDetailName = 'DistributionHotelSingleOrderRefundDetail';
          orderIncomeDetailName = 'DistributionHotelSingleOrderIncomeDetail';
          configType = 5;
          break;
        case 'SelfSupportOrder':
          orderRefundDetailName = 'DistributionHotelSelfSupportOrderRefundDetail';
          orderIncomeDetailName = 'DistributionHotelSelfSupportOrderIncomeDetail';
          configType = 11;
          break;
        case 'financeRefund':
    ...
    getOrderDetailFun(){
      ...
      switch(type){
        ...
        case 'financeRefund':
          ...
          break;
        case 'PurchaseOrder':
          fun = getDistributionHolelPurchaseOrderDetailAsyncRequest;
          break;
        case 'SingleOrder':
          fun = getDistributionHolelSingleOrderDetailAsyncRequest;
          break;
        case 'SelfSupportOrder':
          fun = getDistributionHolelSelfSupportOrderDetailAsyncRequest;
          break;
      }
      return fun
    },
  ...
  methods: {
    ...
    handleChangeOrderOperator(){
      this.$refs.operator.changeOrderOperator(this.orderDetail.order_info.order_id, this.orderDetail.order_info.op_id);
    },
    // 添加收款
