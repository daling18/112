        ...
        <div class="list">
          <el-table :data="tableData" border tooltip-effect="light">
            ...
            <el-table-column
              ...
              :resizable="false">
              <template slot-scope="scope">
                <span :class="scope.row.status | getStatusColor">{{ STATUS_MAP[scope.row.status] }}</span>
              </template>
            </el-table-column>
            <el-table-column
              label="销售人员"
...
import SingleOrderTab from '../plan/components/SingleOrderTab'
import SelfSupportOrderTab from 'src/components/distribution/components/SelfSupportOrderTab'
...
export default {
  ...
  computed: {
    ...
    tabComponent(){
      if(this.isPurchase){
        ...
        case 'planSingleOrder':
          return 'single-order-tab';
        case 'planSelfSupportOrder':
    ...
    orderDetailPerm() {
      switch(this.topMenuConst){
      case 'planSingleOrder':
        return this.$menuPermission('api/selfsupport.localproductorder/op_detail');
      case 'planSelfSupportOrder':
        return this.$menuPermission('api/selfsupport.localproductorder/self_lists');
      case 'distributionMyOrder':
        return this.$menuPermission('api/selfsupport.localproductorder/my_sale_detail');
      default:
  ...
  methods: {
    ...
    handleChangeCreateTime(date) {
      this.searchInput.begin_create_time = date[0] || '';
      this.searchInput.end_create_time = date[1] || '';
    },
    handleSearch(){
      this.getList(true);
