    ...
    <div class="content-list">
      <el-table class="el-table--middle" :data="orderList" border style="width: 100%">
                        ...
                        <tr v-for="(td,tid) in scope.row.tourist_list" :key="tid">
                          ...
                          <td>{{td.name}}
                            <span>
                              ...
                              <router-link :to="{ name: getOrderOption.refundDetailRouteName, query: { order_id: scope.row.order_id }}" 
        ...
        <el-table-column
          ...
          :resizable="false">
            <template slot-scope="scope">
              <router-link :to="{ path: '/saleorder/flight-order/detail', 
                                      query: { order_id: scope.row.order_id }}" 
                                      v-if="$route.name == 'FlightSaleOrder'"
                                      class="look">查看</router-link>
              <router-link :to="{ path: '/singlepro/single-flight-order/detail', 
                                      query: { order_id: scope.row.order_id }}" 
                                      v-if="$route.name == 'FlightSingleOrder'"
                                      class="look">查看</router-link>                   
            </template>
        </el-table-column>
      </el-table>
...
export default {
  ...
  data() {
    return {
      ...
      datepickerOptions: {
        disabledDate:(time)=> {
          return time.getTime() < this.filter.depart_date_start;
          // return time.getTime() < Date.now() - 8.64e7;
        }
      },
      datepickerOptions1: {
        disabledDate:(time)=> {
          return time.getTime() < this.filter.create_date_start;
          // return time.getTime() < Date.now() - 8.64e7;
        }
      },
      pagination:{
  ...
  computed: {
    getOrderOption(){
      let routeName = this.$route.name,
          ...
          requestFn;
          console.log("1000",routeName)
          console.log("111",this.$route)
      switch(routeName){
        case 'FlightSaleOrder':
          type = 'MyOrder';
          detailRouteName = 'FlightSaleOrderDetail'; 
          refundDetailRouteName = 'FlightSaleOrderRefundDetail'; //机票退款详情
          requestFn = getFlightMyOrder; //订单列表（我的订单）
          break;
        case 'FlightSingleOrder':
          type = 'SingleOrder';
          detailRouteName = 'FlightSingleOrderDetail';//机票订单详情
          refundDetailRouteName = 'FlightSingleOrderRefundDetail';
          requestFn = getFlightSingleOrder; //订单列表（订单管理）
          break;
      }
      return {
        type,
  ...
  methods: {
    ...
    initSearchFilter(){
      console.log("000this.$route.query",this.$route.query)
    ...
    //点击查询按钮
    handleSearchRequset(){
      console.log("000this.filter",this.filter)
      let filter = this.filter;
      filter.time = +new Date();
      filter = window.btoa(window.encodeURIComponent(JSON.stringify(filter)));
      console.log("222filter",filter)
      console.log("000path",this.$route.path) ///singlepro/single-flight-order
     ...
     let params = {
      ...
      this.getOrderOption.requestFn({
        ...
        success: (params) => {
          ...
          console.log("111abcparams",params)
