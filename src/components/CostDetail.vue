// src/components/plan/cost/detail.vue
<template>
      ...
      <div class="receive-content">
        ...
        <div class="content-box" v-if="adjustment_detail && adjustment_detail.length > 0">
          ...
          <div v-if="adjustment_detail && adjustment_detail.length > 0">
            ...
            <div class="content-right" v-if="$route.meta.topMenuConst === 'settlement' && $menuPermission('api/finance/adjustment_detail')">
              <router-link v-for="(item, index) in adjustment_detail" :to="{path: 'adjustment-detail', query: {id: item.adjustment_id, type: 2}}" :key="index" class="link2">
                <span class="b-tip">{{ item.item_no }}</span>[调前应收{{ item.front_amount }}]<span class="b-tip">，</span>
              </router-link>
            </div>
          </div>
