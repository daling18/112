<template>
  <div class="wrapper" v-if="detailInfo">
    ...
    <div class="panel">
      <div class="hd">
        <h2>订单信息</h2>
        <span><span>支付渠道：{{detailInfo.order_info.pay_channel_code}}</span><span>第三方流水号：{{detailInfo.order_info.channel_req_no}}</span></span>
      </div>
      <div class="bd">
            ...
            <td>
              <el-dropdown trigger="click" v-if="detailInfo.order_info.allow_operation && detailInfo.order_info.status_code != 3">
                ...
                <el-dropdown-menu slot="dropdown">
                  ...
                  <el-dropdown-item v-if="(($menuPermission('singlepro/singleorder/ticketorder/cancelorder') && orderType == 'SingleOrder') || 
                    ($menuPermission('saleorder/ticketorder/cancelorder') && orderType == 'MyOrder')) && 
                    detailInfo.order_info.status_code == 11">
                    <a href="javascript:;" @click="requestCancelTicketOrderHandler">取消订单</a>
                  </el-dropdown-item>
                  <el-dropdown-item v-if="(($menuPermission('saleorder/ticketorder/onlinepay') && orderType == 'MyOrder')) && 
                    detailInfo.order_info.status_code == 11">
                    <a href="javascript:;" @click="handleOnlinePay">在线支付</a>
                  </el-dropdown-item>
                  <el-dropdown-item v-if="(($menuPermission('singlepro/singleorder/ticketorder/aplrefundticket') && orderType == 'SingleOrder') ||
                    ($menuPermission('saleorder/ticketorder/aplrefundticket') && orderType == 'MyOrder')) && 
                    detailInfo.order_info.status_code == 0 || 
                    detailInfo.order_info.status_code == 12">
                    <a href="javascript:;" @click="setRefundTicketHandler(detailInfo.order_info)">申请退票</a>
                  </el-dropdown-item>
                  <el-dropdown-item v-if="(($menuPermission('singlepro/singleorder/ticketorder/chgmobile') && orderType == 'SingleOrder') || 
                    ($menuPermission('saleorder/ticketorder/chgmobile') && orderType == 'MyOrder')) && 
                    detailInfo.order_info.status_code == 0">
                    <a href="javascript:;" @click="setContactTicketHandler(detailInfo.order_info)">修改手机号</a>
                  </el-dropdown-item>
                  </template>
                  <template v-if="orderType == 'SelfSupportOrder'">
                  <el-dropdown-item v-if="$menuPermission('singlepro/selforder/tickertorder/coforder') && detailInfo.order_info.status_code == 12">
                    <a href="javascript:;" @click="requestConfirmTicketOrderHandler">确认订单</a>
                  </el-dropdown-item>
                  <el-dropdown-item v-if="$menuPermission('singlepro/selforder/tickertorder/veri') && detailInfo.order_info.status_code == 0">
                    <a href="javascript:;" @click="requestVerificationTicketOrderHandler">验证门票</a>
                  </el-dropdown-item>
                  <el-dropdown-item v-if="$menuPermission('ssettlement/recpay/recei/addrecei') && 
                    detailInfo.order_info.status_code != 11 && 
                    detailInfo.order_info.status_code != 12 &&
                  ...
                  <el-dropdown-item v-if="(($menuPermission('singlepro/singleorder/ticketorder/getincodet') && orderType == 'SingleOrder') || 
                    ($menuPermission('singlepro/selforder/tickertorder/getincodet') && orderType == 'SelfSupportOrder') ||
                    ($menuPermission('saleorder/ticketorder/getincodet') && orderType == 'MyOrder'))
                    && detailInfo.order_info.status_code != 3 && 
                    detailInfo.order_info.status_code != 11 &&
                    detailInfo.order_info.status_code != 12">
                    <router-link :to="{ name: orderTypeRouterName.orderIncomeDetailName, 
                      query: { bid: detailInfo.bill_id, type:orderTypeRouterName.configType } }">查看收入</router-link>          
                  </el-dropdown-item>
                  <el-dropdown-item v-if="($menuPermission('singlepro/singleorder/ticketorder/orderprin') && orderType == 'SingleOrder') || 
                    ($menuPermission('singlepro/selforder/tickertorder/orderprin') && orderType == 'SelfSupportOrder')">
    ...
    <online-pay ref="onlinePay" :order-id="orderNo" order-resource="6" 
      v-if="(($menuPermission('saleorder/ticketorder/onlinepay') && orderType == 'MyOrder')) && detailInfo &&
                    detailInfo.order_info.status_code == 11"/>
  ...
  computed: {
    orderType(){
      let routeName = this.$route.name,
          type = '';
      switch(routeName){
        case 'TicketSaleOrderDetail':
        case 'financeTicketMyOrderDetail':
          type = 'MyOrder';
          break;
        case 'TicketSingleOrderDetail':
          type = 'SingleOrder';
          break;
        case 'TicketSelfSupportOrderDetail':
          type = 'SelfSupportOrder';
          break;
        case 'settlementRefdealTicketOrderDetail':
          type = 'SingleOrder';
          break;
      }
      return  type;
    ...
    orderTypeRouterName(){
      ...
      switch(type){
        case 'MyOrder':
          orderRefundDetailName = 'TicketSaleOrderRefundDetail';
          orderIncomeDetailName = 'TicketSaleOrderIncomeDetail';
          configType = 19;
          break;
        case 'SingleOrder':
          orderRefundDetailName = 'TicketSingleOrderRefundDetail';
          orderIncomeDetailName = 'TicketSingleOrderIncomeDetail';
          configType = 6;
          break;
