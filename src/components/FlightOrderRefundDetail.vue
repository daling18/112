...
<div class="wrapper">
  <div class="system-tab">
    <ul class="tab-box">
      <template>
        <router-link :to="{ name: orderDetailName.routeName, query: { order_id: $route.query.order_id }}" tag="li">
          订单详情
        </router-link>
        <router-link
          v-cloak
          :to="{ name: orderDetailName.orderEndorseDetailName, query: { order_id: $route.query.order_id }}"
          v-if="change_detail == 0"
          tag="li"
        >变更单详情</router-link>
        <li class="on" v-if="orderData.length > 0">退款详情</li>
      </template>
    </ul>
  </div>
      ...
      <div class="module-table module-table-border">
        <table v-for="(obj,idx) in orderData" :key="idx" class="refund-table">
          ...
          <tr>
            ...
            <td class="tc">
              <el-button type="text" 
                v-if="$menuPermission('singlepro/singleorder/flightorder/loss') && 
                obj.refund_status == 3 && 
...
export default {
  ...
  computed:{
    orderType(){
      ...
      switch(routeName){
        case 'FlightSaleOrderRefundDetail':
          type = 'MyOrder';
          break;
        case 'FlightSingleOrderRefundDetail':
          type = 'SingleOrder';
          break;
    ...
    orderDetailName(){
      ...
      return {routeName,orderEndorseDetailName}
    },
      ...
      this.getOrderDetailFun({
        ...
        success: (params) => {
          ...
          if(code == 0){
            ...
            this.change_detail = asyncObj.data.order_info.change_detail
          }else{
