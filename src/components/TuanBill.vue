<template>
<!-- application\index\view\dev\src\components\plan\control\bill.vue -->
  <div class="bill-wrap">
    <div class="bill-top-wrap">
      <el-button type="primary" @click="lockDialog = true" v-if="lockBillPerm">锁账生成团账</el-button>
      <el-button type="primary" @click="canUpdatebill" v-if="updateBillPerm">申请调账</el-button>
    </div>
    <div class="bill-con">
      ...
      <div :class="applicationAccount ? 'applictionAccount-wrap' : ''">
        <div class="bill-table-wrap">
          <div class="bill-table-tit">
            收入明细
            <el-button class="right-btn" v-if="tuanExportIncomeWordPerm" @click="uploadDoc('income')">导出收入结算表</el-button>
          </div>
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <!-- <router-link
                    ...
                    v-if="$menuPermission('api/tuan/op_income_info') && $route.meta.topMenuConst === 'tuancontro'">
                    查看
                  </router-link>
                  <router-link
                  ...
                  <router-link
                    v-if="incomeDetailPerm"
                    :to="{ path: incomeDetailPath, query: {aid:data.tuan_info.a_id, bid:scope.row.id}}">
                    查看
                  </router-link>
                  <div v-else>--</div>
                </template>
        ...
        <div class="bill-table-wrap">
          <div class="bill-table-tit">
            成本明细
            <el-button class="right-btn" v-if="tuanExportCostWordPerm" @click="uploadDoc('cost')">导出成本结算表</el-button>
          </div>
              ...
              <el-table-column
                ...
                <template slot-scope="scope">
                  <!-- <router-link
                    ...
                    v-if="$menuPermission('api/tuan/bill_pay_detail') && $route.meta.topMenuConst === 'tuancontro'">
                    查看
                  </router-link>
                  <router-link
                  ...
                  <router-link
                    v-if="costDetailPerm"
                    :to="{ path: costDetailPath, query: {aid:data.tuan_info.a_id, id:scope.row.id}}">
                    查看
                  </router-link>
                  <div v-else>--</div>
                </template>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <!-- <div v-if="scope.row.id || scope.row.approval_id"> -->
                ...
                <router-link v-else-if="scope.row.id && $route.meta.topMenuConst === 'tuancontro'" :to="{path: '/control/adjustment-detail', query: {id: scope.row.id, type: 2}}">查看调整</router-link> -->
                ...
                <router-link v-else-if="scope.row.approval_id && $route.meta.topMenuConst === 'tuancontro'" :to="{path: '/control/control-approval-detail', query: {id: scope.row.approval_id}}">审批单</router-link> -->
                <!-- <div v-else>--</div> -->
              <!-- </div> -->
              <router-link v-if="adjustmentDetailPerm(scope.row)" :to="{path: adjustmentDetailPath, query: {id: scope.row.id, type: 2}}">查看调整</router-link>
              <router-link v-if="approvalDetailPerm(scope.row)" :to="{path: '/approval/submit/detail', query: {id: scope.row.approval_id}}">审批单</router-link>
              <span v-else-if="!adjustmentDetailPerm(scope.row)">--</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
...
export default {
  ...
  computed: {
    permission(){
      ...
      switch(routeName){
        case 'cControlBill':
          return {
            lockBillPerm: this.$menuPermission('tuancontro/line/lockaccounts'),
            updateBillPerm: this.$menuPermission('tuancontro/line/tuanaccop/applychgaccount'),
            tuanExportIncomeWordPerm: this.$menuPermission('tuancontro/line/tuanaccop/outrecei'),
            incomeDetailPerm: this.$menuPermission('tuancontro/line/incomelist/getincodet'),
            incomeDetailPath: '/tuancontro/bill/income-detail',
            tuanExportCostWordPerm: this.$menuPermission('tuancontro/line/tuanaccop/outincome'),
            costDetailPerm: this.$menuPermission('tuancontro/line/costlist/detail'),
            costDetailPath: '/tuancontro/cost/detail',
            adjustmentDetailPath: '/tuancontro/bill/adjustment-detail',
            adjustmentDetailPerm: this.$menuPermission('tuancontro/line/tuanaccop/getchgaccount'),
            applicationActionPath: '/tuancontro/billIncomeCostUpdate'
          }
        case 'TuancontroSingleBill':
          return {
            lockBillPerm: this.$menuPermission('tuancontro/single/lockfund'),
            updateBillPerm: this.$menuPermission('tuancontro/single/tuanaccop/applychgaccount'),
            tuanExportIncomeWordPerm: this.$menuPermission('tuancontro/single/tuanaccop/outrecei'),
            incomeDetailPerm: this.$menuPermission('tuancontro/single/incomelist/getincodet'),
            incomeDetailPath: '/tuancontro/single/bill/income-detail',
            tuanExportCostWordPerm: this.$menuPermission('tuancontro/single/tuanaccop/outincome'),
            costDetailPerm: this.$menuPermission('tuancontro/single/costlist/detail'),
            costDetailPath: '/tuancontro/single/cost/detail',
            adjustmentDetailPath: '/tuancontro/single/bill/adjustment-detail',
            adjustmentDetailPerm: this.$menuPermission('tuancontro/single/tuanaccop/getchgaccount'),
            applicationActionPath: '/tuancontro/single/billIncomeCostUpdate'
          }
      }
    },
    topMenuConst() {
    ...
    isOperate() {
      return this.topMenuConst === 'tuancontro';
    },
    lockBillPerm() {
      const perm = this.isOperate && this.permission.lockBillPerm
      const tuanInfo = this.data && this.data.tuan_info
      return perm && tuanInfo.lock_status === 1 && tuanInfo.lock_fund !== 1
    },
    updateBillPerm() {
      const perm = this.isOperate && this.permission.updateBillPerm
      const tuanInfo = this.data && this.data.tuan_info
      return perm && tuanInfo.lock_status === 1 && tuanInfo.lock_fund === 1
    },
    tuanExportIncomeWordPerm() {
      switch(this.topMenuConst) {
      case 'settlement':
        return this.$menuPermission('settlement/tuanac/gettuanac/expincofund')
      case 'tuancontro':
        return this.permission.tuanExportIncomeWordPerm
      default:
        return false;
      }
    },
    incomeDetailPerm() {
      ...
      case 'tuancontro':
        return this.permission.incomeDetailPerm
      default:
        return false;
      }
    },
    incomeDetailPath() {
      ...
      case 'tuancontro':
        return this.permission.incomeDetailPath
      default:
        return false;
      }
    },
    tuanExportCostWordPerm() {
      switch(this.topMenuConst) {
      case 'settlement':
        return this.$menuPermission('settlement/tuanac/gettuanac/expcostfund')
      case 'tuancontro':
        return this.permission.tuanExportCostWordPerm
      default:
        return false;
      }
    },
    costDetailPerm() {
      switch(this.topMenuConst) {
      case 'settlement':
        return this.$menuPermission('settlement/tuanac/gettuanac/getcostdet')
      case 'tuancontro':
        return this.permission.costDetailPerm
      default:
        return false;
      }
    },
    costDetailPath() {
      ...
      case 'settlement':
        return 'cost-detail'
      case 'tuancontro':
        return this.permission.costDetailPath
      default:
        return false;
      }
    },
    adjustmentDetailPath() {
      ...
      case 'tuancontro':
        return this.permission.adjustmentDetailPath
      default:
        return false;
      }
    }
  ...
  methods: {
    adjustmentDetailPerm(data) {
      if (!data.id) {
        return false
      }
      switch(this.topMenuConst) {
      case 'settlement':
        return this.$menuPermission('settlement/tuanac/gettuanac/adjustmentdetail')
      case 'tuancontro':
        return this.permission.adjustmentDetailPerm
      default:
        return false;
      }
    },
    approvalDetailPerm(data){
      return this.$menuPermission('usercenter/approval/detail') && data.approval_id
    },
    getGroupBillDetail() {
      let params = {
        data: {
    ...
    applicationAction() {
      ...
      //   return false;
      // };
      if (this.$route.meta.topMenuConst === 'tuancontro') {
        this.$router.push({
          path: permission.applicationActionPath,
          query: {
            tuan_id: this.id,
            bill_ids
          }
