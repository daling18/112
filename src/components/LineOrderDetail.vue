// src\components\distribution\LineOrderDetail.vue
<template>
  <div class="details">
    ...
    <div v-if="orderDetailData" class="cont">
      <div class="panel">
        ...
        <div class="bd">
          <el-table class="el-table--middle" :data="orderDetailData.order_info" border style="width: 100%">
            ...
            <el-table-column align="center" prop="confirm_status" width="80px" label="订单状态">
              <template slot-scope="scope">{{scope.row.confirm_status | mapOrderStatus}}</template>
            </el-table-column>
            <el-table-column align="center" prop="departure_date" width="100px" label="出团日期" />
            <el-table-column
              header-align="center"
              align="right"
            ...
            <template v-if="!isShopLine">
              <el-table-column min-width="80" label="联系人" prop="distributor_real_name" ></el-table-column>
              <el-table-column min-width="80" label="联系电话" prop="distributor_mobile"></el-table-column>
            </template>
            <template
              v-if="isShopLine"
            >
      ...
      <div class="panel">
        ...
        <div class="neck-txt">
          ...
          <el-button class="op-btn" v-if="canAddOrderDetail" type="primary" @click="handleAddPriceItem('show')">
            + 新增项目
          </el-button>
          ...
          <el-table style="width: 100%" :data="materialInfo.list" border>
            ...
            <el-table-column label="操作" align="center">
              <template slot-scope="scope">
                ...
                <template v-else-if="isNoMaterialOperate(scope.row.edit)">--</template>
                <template v-else v-for="(edit, i) in scope.row.edit">
                  <a v-if="edit == 5 && isShowMaterialOperate(edit, scope.row)" :key="i" download :href="materialInfo.edit_data.notification_url">
                    下载出团通知书
                  </a>
    ...
    <el-dialog
      title="修改合同单价"
      :visible.sync="editHeTongPriceDialogVisible"
      size="tiny"
      center>
      <el-form label-width="70px" class="contracForm">
        <div class="itemForm">
        ...
        <div class="itemForm">
              <span style="margin-left: 60px;">成人单价</span>
              <el-input class="width1" v-model="contracPrices.adult.price" :disabled="contracPrices.adult.disabled" @keyup.native="editMsgType('adult')"></el-input>
              <span>
                儿童单价
              </span>
              <el-input class="width1" v-model="contracPrices.child.price" :disabled="contracPrices.child.disabled" @keyup.native="editMsgType('child')"></el-input>
        </div>
        <div class="itemForm" style="margin-left: 70px;">（修改销售总价后，电子合同单价*人数应等于总价，单价可以设置为平均单价）</div>
        <div class="itemForm">
          <div class="label required">
            成团人数与不成团的约定
          </div>
        </div>
        <div class="itemForm" style="margin-left: 70px;">本团最低 {{contracPrices.min_people}} 人成团，如不能成团，旅游者按以下哪种方式解决：</div>
        <el-form-item required>
          <el-radio-group v-model="contracPrices.tuan_constraint">
            <el-radio :label="1" style="display: block; margin: 10px 0">出境社委托<el-input style="width: 100px;" v-model="contracPrices.company_name" :disabled="contracPrices.tuan_constraint != 1"></el-input>出境社履行合同</el-radio>
        ...
        <div class="itemForm" style="margin-left: 70px;">出境社提示旅游者购买人身意外伤害保险，旅游者可以做以下选择：</div>
        <el-form-item required>
        ...
        <div class="itemForm" style="margin-left: 70px;">未尽事宜，经旅游者和出境社双方协商一致，可以列入补充条款：</div>
        <el-form-item>
          <el-input 
            type="textarea"
            :rows="2"
            :placeholder="agreed_matters1"
            v-model="contracPrices.agreed_matters">
          </el-input>
        </el-form-item>
      </el-form>
      <span slot="footer">
        <el-button type="primary" @click="submitEditPrice">保 存</el-button>
        <el-button @click="editHeTongPriceDialogVisible = false">取 消</el-button>
      </span>
    </el-dialog>

    <!-- <el-dialog title="预览合同" :visible.sync="previewVisible" width="60%" >
    ...
    <online-pay
      ...
      @finish="handleLoadPayFinish"
    />
    <!-- <router-link target="_blank" :to="{path:'/saleorder/contract-preview',query:{id:'1'}}">预览合同</router-link> -->
  </div>
</template>
...
import {
  ...
  reGenerateContract,
  getContractPrices,
  contractPricesChange
} from 'src/api/tuan';
import {
  setPaymentAmount,
  setOrderDetailAccount,
  getOrderShopDetail
} from 'src/api/order';
...
import OrderPriceItem from './components/OrderPriceItem.vue';
...
export default {
  ...
  filters: {
    ...
    mapOrderStatus (status) {
      //订单确认状态：-1=已取消 0=待处理 1=已占位 2=已确认 9=已完成，二次确认,刚下完单, confirm_status=0
  ...
  data() {
    ...
    return {
      ...
      tuanNo: this.$route.params.parent_id,
      orderInfo: {},
      ...
      orderType: this.$route.name == 'PurchaseOrderLineOrderDetail' ? 1:2,
      // 编辑游客信息相关变量 begin
  ...
  computed: {
    contractPreview(){
      return this.$menuPermission('saleorder/contract-preview');
    },
    isShopLine() {
      return this.$route.name === 'ShopOrderLineOrderDetail';
    },
    topMenuConst() {
    ...
    isOperator() {
      // 是否计调操作
      return this.topMenuConst == 'touristroute';
    },
    isSale() {
      // 是否销售操作
      return this.topMenuConst == 'saleorder' || this.topMenuConst == 'purchaseorder';
    },
    isBOMTrip() {
    ...
    canAddOrderDetail() {
      const permission = this.operatorPriceDetailAddPerm;
    ...
    showEditTouristCol() {
      return (this.isSale && !this.isCancelOrder);
    },
    tuituanDetailPerm() {
      switch (this.topMenuConst) {
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/getbackdetail');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/getbackdetail');
      case 'settlement':
        return this.$menuPermission('settlement/getbackdetail');
      // case 'finance':
      //   return this.$menuPermission('api/finance/fin_get_cancel_detail');
      case 'touristroute':
      default:
        return this.$menuPermission('touristroute/lineorder/lineorder/getbackdetail');
      }
    },
    tuituanDetailUrl() {
      return '../../order-refund';
    ...
    uploadPagerContractPerm() {
      switch (this.topMenuConst) {
      case 'saleorder': // 销售订单直客订单签约纸质合同
        return this.$menuPermission('saleorder/lineorder/getorderdetail/suresignorder');
      // case 'purchaseorder':
      //   return this.$menuPermission('saleorder/lineorder/getorderdetail/suresignorder');
      default:
        return false;
    ...
    pagerContractDetailPerm() {
      ...
      // case 'finance':
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/materialop/upvisamat');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/materialop/upvisamat');
      case 'settlement':
        return this.$menuPermission('settlement/materialop/upvisamat');
      default:
        return false;
      }
    },
    ...
    addInvoicePerm() {
      return this.$menuPermission('settlement/invoice/income/edit');
      // switch (this.topMenuConst) {
      // case 'distribution':
      // case 'saleorder':
    ...
    addInvoiceUrl() {
      return { path: '/settlement/invoice/income/add', query: {id: this.bills_id, type: 1} }
      // switch (this.topMenuConst) {
      // case 'distribution':
      // case 'saleorder':
      //   return {
    ...
    invoiceDetailUrl() {
      return { path: '/settlement/invoice/income/detail', query: {bid: this.bills_id, type: 1} }
      // switch (this.topMenuConst) {
      // case 'distribution':
      // case 'saleorder':
      //   return {
    ...
    chakanVisaPerm() {
      switch (this.topMenuConst) {
      case 'touristroute':
        return this.$menuPermission('touristroute/lineorder/lineorder/material/visadetail');
      case 'settlement':
        return this.$menuPermission('settlement/material/upvisamat');
      // case 'finance':
    ...
    getOrderDetailFunc() {
      console.log('top menu const: ', this.topMenuConst)
      switch (this.topMenuConst) {
        case 'touristroute':
    ...
    saveOrderGatheringPerm() {
      return this.$menuPermission('settlement/recpay/recei/addrecei');
    ...
    saveOrderGatheringFunc() {
      ...
      // console.log(this.topMenuConst);
      if(this.topMenuConst == 'purchaseorder'){
        return setOrderDetailAccount
      }else{
        if(this.orderInfo.order_from == 3){
          return setOrderDetailAccount
        }
        return financeSaveOrderGathering
      }

      console.log(this.orderInfo.order_from);
    },
    incomeInfoPerm() {
      switch (this.topMenuConst) {
      case 'touristroute':
        return this.$menuPermission('tuancontro/line/incomelist/getincodet');
      case 'settlement':
      case 'saleorder':
      case 'purchaseorder':
    ...
    incomeInfoUrl() {
      ...
      switch (this.topMenuConst) {
      case 'touristroute':
        path = '/tuancontro/income/income-detail';
        break;
      case 'settlement':
      case 'saleorder':
      case 'purchaseorder':
        path = '/settlement/tuan-bill/income-detail';
      }
      return {
        path: path,
    ...
    saleConfirmOrderPerm() {
      ...
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/getorderdetail/sureorder');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/getorderdetail/suresignorder');
      default:
        return false;
    ...
    doCancelTuanTouristPerm() {
      switch (this.topMenuConst) {
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/getorderdetail/applyback');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/getorderdetail/applyback');
      default:
        return false;
      }
    },
    ...
    editTouristInfoPerm() {
      switch (this.topMenuConst) {
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/getorderdetail/edittouristinfo');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/getorderdetail/edittouristinfo');
      default:
        return false;
      }
    },
    ...
    cancelOrderPerm() {
      switch (this.topMenuConst) {
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/getorderdetail/cancelorder');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/getorderdetail/cancelorder');
      default:
        return false;
      }
    },
    ...
    //   switch (this.topMenuConst) {
    //   case 'purchaseorder':
    //     return this.$menuPermission('purchaseorder/lineorder/getorderdetail/contractpriceschange');
    //   default:
    //   return false;
    //   }
    // },
    ...
    createElectronicContractPerm() {
      switch (this.topMenuConst) {
      case 'saleorder': // 订单签约
        return this.$menuPermission('saleorder/lineorder/getorderdetail/suresignorder,purchaseorder/lineorder/materialop/genelcon');
      case 'purchaseorder': // 材料操作-生成合同
        return this.$menuPermission('purchaseorder/lineorder/materialop/genelcon');
      default:
        return false;
      }
    },
    ...
    dlElectronicContractPerm() {
      ...
      // case 'finance':
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/materialop');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/materialop');
      default:
        return false;
      }
    },
    ...
    onlinePayPerm() {
      ...
      // case 'finance':
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/getorderdetail/onlinepay');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/getorderdetail/onlinepay');
      default:
        return false
      }
    },
    distrGenerateDownloadContractPerm() {
      ...
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/materialop/genelcon');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/materialop/genelcon');
      default:
        return false;
      }
    },
    ...
    operatorPriceDetailAddPerm() {
      switch (this.topMenuConst) {
      case 'touristroute':
        return this.$menuPermission('touristroute/lineorder/lineorder/getorderdetail/addpricepro');
      default:
        return false;
      }
    },
    ...
    dlQrjPerm() {
      ...
      //   return apiStoreSaleConfirmOrder;
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/materialop/downloadsure');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/materialop/downloadsure');
      default:
        return false;
      }
    },
    dlQrj() {
      ...
      case 'operator':
        return (
          '/bom/api/tuan/save_order_confirm_pdf?id=' + this.orderInfo.id
        );
      case 'purchaseorder':
      case 'saleorder':
        return (
    ...
    dlCttzPerm() {  // 下载出团通知
      ...
      //   return apiStoreSaleConfirmOrder;
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/lineorder/materialop/downlaodouttuan');
      case 'saleorder':
        return this.$menuPermission('saleorder/lineorder/materialop/downlaodouttuan');
      default:
        return false;
      }
    },
    offlineCollection(){
      switch (this.topMenuConst) {
      case 'purchaseorder':
        return this.$menuPermission('purchaseorder/offline-collection');
      case 'saleorder':
        return this.$menuPermission('saleorder/offline-collection');
      default:
        return false;
      }
    }
  ...
  methods: {
    editHeTong(data){
      ...
      getContractPrices({
        ...
        success:(res)=>{
          console.log(res,'getContractPrice')
          let { code, data, msg } = res['data'];
          if( code === 0 && data){
              this.contracPrices = data;
              this.agreed_matters1=data.agreed_matters
    ...
    editMsgType(type){
      let intNum = new RegExp("^[1-9][0-9]*$"),
         adultPrice = this.contracPrices.adult.price,
          childPrice = this.contracPrices.child.price;
      
      if(type == 'adult'){
        this.contracPrices.adult.price = adultPrice.replace(/[^\d]/g,'')
       
      }else{
        this.contracPrices.child.price = childPrice.replace(/[^\d]/g,'')
      }

    },
    submitEditPrice(){
      
      if(this.contracPrices.adult.price == 0 && this.contracPrices.child.price == 0){
        this.$message.warning('成人单价和儿童单价不能同时为空或0')
        return
      }

      let params = {
          order_no: this.orderDetailData.order_info[0].order_no,
          adult_price: this.contracPrices.adult.price *1,
          child_price: this.contracPrices.child.price *1,
          tuan_constraint: this.contracPrices.tuan_constraint,
          accident_injury: this.contracPrices.accident_injury,
          agreed_matters: this.contracPrices.agreed_matters,
          company_name: this.contracPrices.company_name
        }
      ...
      contractPricesChange({
        ...
        success:(res)=>{
          ...
          this.editHeTongPriceDialogVisible = false;
        }
      })
    },
    emptyFunc(msg){
    ...
    getOrderDetail() {
      let params = {
        data: {
          id: this.$route.query.id || this.$route.params.order_no || '',
          data_config_type: this.dataConfigType,
          tuan_id: this.$route.params.parent_id,
          order_no: this.$route.params.order_no // 不了解业务，先新增字段
        }
      };
      ...
      this.getOrderDetailFunc({
        ...
        success: params => {
          ...
          if (code == 0) {
            ...
            } else if (typeof data.order_price_detail === 'object') {
              // 生成operator字段
              var arr = [];
              Object.keys(data.order_price_detail).forEach(function(key){
                  console.log(key,data.order_price_detail[key]);
                // 处理operator为 - 的记录返回的负数，转成正数，因为页面都是使用正数进行计算
                if (data.order_price_detail[key].unit_price < 0) {
                  data.order_price_detail[key].unit_price = Math.abs(data.order_price_detail[key].unit_price);
                  data.order_price_detail[key].total_amount = Math.abs(data.order_price_detail[key].total_amount);
                  data.order_price_detail[key].rmb_unit_price = Math.abs(data.order_price_detail[key].rmb_unit_price);
                  data.order_price_detail[key].rmb_total_amount = Math.abs(data.order_price_detail[key].rmb_total_amount);
                  data.order_price_detail[key].operator = '-';
                } else {
                  data.order_price_detail[key].operator = '+';
                }
                arr.push(data.order_price_detail[key])
              });

              data.order_price_detail = arr;
            }
    ...
    isShowOrderOperate(edit, order) {
      ...
      case 7:
        return this.$menuPermission('touristroute/lineorder/lineorder/getorderdetail/sureposition');
      case 9:
        return this.doCancelTuanTouristPerm;  // 退款和退团通过同一个权限字符串控制
      case 10:
        return this.onlinePayPerm; //this.$menuPermission('api/pay/pay_order')
        // 营业部订单，且未设置支付金额的不显示在线支付按钮
        // && (!this.isStore || this.orderInfo.preset_payment_amount);
      case 11:
        return this.$menuPermission('purchaseorder/lineorder/getorderdetail/contractpriceschange');//修改合同单价
      case 12:
          ...
          if (code == 0) {
            ...
            if (this.tuituanDetailPerm) {
              this.$router.push({
                ...
                query: { ono: this.orderInfo.order_no, tid: this.tuanNo }
              });
            }else{
    ...
    exitOrderSubmit() {
      ...
      this.exitOrderSubData.tuan_id = this.tuanId;
