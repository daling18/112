      ...
      <div class="content-list">
        <div class="list">
          <el-table :data="tableData" class="el-table--middle" border style="width: 100%">
            ...
            <el-table-column label="账单操作" align="center" width="120" fixed="right">
              <template slot-scope="scope">
                <div v-show="scope.row.lock_status==1">
                  <div class="switch">
                    <el-switch
                      ...
                      active-color="#666"
                      off-color="#b10c79"
                      :disabled="!$menuPermission('api/finance/lock_fund') && !$menuPermission('api/finance/unlock_fund')"/>
                  </div>
                </div>
            ...
            <el-table-column label="查看" align="center" fixed="right" width="100">
              <template slot-scope="scope">
                <template v-if="$menuPermission('api/tuan/tourist_lists')||$menuPermission('api/tuan/order_lists')||$menuPermission('api/tuan/op_log')||$menuPermission('api/finance/bill_detail')">
                  <el-dropdown>
                    <span class="el-dropdown-link">
                      查看<i class="el-icon-caret-bottom el-icon--right"/>
                    </span>
                    <el-dropdown-menu slot="dropdown">
                      <el-dropdown-item v-if="$menuPermission('api/tuan/tourist_lists')"><router-link :to="{name:'fControlRecruiting', params:{id:scope.row.a_id,type:constDataConfigType}}">游客名单</router-link></el-dropdown-item>
                      <el-dropdown-item v-if="$menuPermission('api/tuan/order_lists')"><router-link :to="{path:'/ftuan-list',query:{id:scope.row.a_id,lock_status:scope.row.lock_status,type:constDataConfigType}}">所有订单</router-link></el-dropdown-item>
                      <el-dropdown-item v-if="$menuPermission('api/tuan/op_log')"><router-link :to="{name:'fControlOperating',params:{id:scope.row.a_id,type:constDataConfigType}}">操作记录</router-link></el-dropdown-item>
                      <el-dropdown-item v-if="$menuPermission('api/finance/bill_detail')" >
                        <router-link :to="{path:'/fcontrol/bill',query:{id:scope.row.a_id,type:constDataConfigType}}">团队账单</router-link>
                      </el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                </template>
                ...
                <!-- <div class="tool">
                  <router-link :to="{path:'',query:{id:scope.row.id}}">团队行程</router-link>
                </div> -->
              </template>
            </el-table-column>
          </el-table>
...
export default {
  ...
  methods: {
    ...
    //   if (param.lock_fund_name == 'unlock_fund') {
    ...
    //         if (data.code == '0') {
    //           param.lock_fund_name='lock_fund';
    //           param.lock_fund=1;
    ...
    //     });
    //   }
    //   if(param.lock_fund_name == 'lock_fund'){
    //     financeUnlockAction({
    //       params: params,
    //       success: (params) => {
    //         let data = params.data;
    ...
    //         if (data.code == '0') {
    //           param.lock_fund_name ='unlock_fund';
    //           param.lock_fund =0;
    //           this.$message.success(data.msg);
    //         } else {
    //           this.$message.error(data.msg);
