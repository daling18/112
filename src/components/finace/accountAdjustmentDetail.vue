<template>
  <div class="bill-wrap">
    <div class="bill-table-tit">
      基本信息
      <el-button class="right-btn" @click="goBack">返回</el-button>
      <el-button type="primary" @click="openDialog(3)" class="right-btn" v-if="$route.meta.topMenuConst === 'finance' && $route.query.type == 1 && $menuPermission('api/finance/adjustment_verify')">审核不通过</el-button>
      <el-button type="primary" @click="openDialog(2)" class="right-btn" v-if="$route.meta.topMenuConst === 'finance' && $route.query.type == 1 && $menuPermission('api/finance/adjustment_verify')">审核通过</el-button>
    </div>
    ...
    <el-dialog :title="status === 2 ? '审核通过' : '审核不通过'" :visible.sync="verifyType">
      <el-form
          label-width="80px">
          <el-form-item label="备注" :required="status === 2 ? false : true">
            <el-input v-model="remark" type="textarea"></el-input>
          </el-form-item>
        </el-form>
      <span slot="footer">
        <el-button @click="verifyType = false">取 消</el-button>
        <el-button type="primary" @click="adjustmentVerify">确 定</el-button>
      </span>
    </el-dialog>
  </div>
</template>
<script>
import {financeAdjustmentDetail, adjustmentVerify} from 'src/api/finance'
