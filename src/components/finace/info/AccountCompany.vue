<template>
  <div class="wrapper">
    <div class="system-tab">
      <ul class="tab-box">
        <li class="on">公司账户</li>
        <router-link tag="li" v-if="$menuPermission('api/peeraccount')" :to="{path:'/finace/account/peer'}">
          同行账户
        </router-link>
      </ul>
      <div class="btn">
        <el-button type="primary" size="long" @click="edit()" v-if="$menuPermission('api/finance/add_payment_account')">+ 添加往来账户</el-button>
      </div>
    </div>
    <!-- system-search -->
    <div class="system-search">
      ...
      <div class="search-item">
        ...
        <el-input v-model="searchData.account_no" placeholder="银行卡号/支付平台账号"/>
      </div>
        ...
        <div class="list">
          <el-table :data="tableData" border style="width: 100%">
            ...
            <el-table-column label="操作" align="center" width="120" fixed="right">
              <template slot-scope="scope">
                <template v-if="$menuPermission('api/finance/edit_payment_account')"><el-button type="text" @click="edit(scope.row)">编辑</el-button></template>
                <template v-else>
        ...
        <el-form :model="form">
          ...
          <div v-show="form.account_type=='3'">
            <el-form-item label="银行卡号" :label-width="formLabelWidth" required>
              <el-input v-model="form.bank_no" placeholder="请输入银行卡号" auto-complete="off"/>
            </el-form-item>
          </div>
          <div v-show="form.account_type=='4'">
            <el-form-item label="银行卡号" :label-width="formLabelWidth" required>
              <el-input v-model="form.other_no" placeholder="请输入银行卡号" auto-complete="off"/>
            </el-form-item>
          </div>
        </el-form>
      </div>
...
<script>
import { getPaymentAccountList,editPaymentAccount,addPaymentAccount } from 'src/api/api';
