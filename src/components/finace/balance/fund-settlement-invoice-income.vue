  ...
  <div class="wrapper">
    <div class="system-tab">
      <ul class="tab-box">
        <li class="on">收入发票</li>
        <router-link
          v-if="$menuPermission('api/invoice/tonghang_settle_cost')"
          tag="li"
          :to="{path:'/fund-settlement/invoice-cost', query: $route.query}"
        >
          成本回票
        </router-link>
      </ul>
      <div class="btn" v-if="$menuPermission('api/invoice/finance_add_income')">
        <router-link
          class="el-button el-button--primary" 
          :to="{path: '/fund-settlement/invoice-income/add', query: {parent_id: $route.query.id}}"
        >+ 添加开票</router-link>
      </div>
    </div>
    ...
    <div class="system-content">
      <div class="content-list" style="margin-top:15px;">
        <invoice-income-table prefixUrl="/fund-settlement/invoice-income" :tableData="tableData" :isUsedMap="IS_USED_MAP" @cancel="getTableData(true)" />
      </div>
    </div>
...
<script>
import {tonghangSettleIncome}from 'src/api/invoice';
