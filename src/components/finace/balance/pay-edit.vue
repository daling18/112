    ...
    <div class="system-content">
      <div class="mod-form">
        ...
        <div class="form-content">
          ...
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="170px" class="demo-ruleForm">
            <template v-if="isPaymentListPayEdit">
...
export default {
  ...
  computed:{
    isPaymentListPayEdit(){
      if(this.$route.name == 'paymentListPay-edit'){
        return true;
      }
    },
    orgination: {
  ...
  methods: {
    ...
    searchTuan() {
      if(this.isPaymentListPayEdit && this.ruleForm.settlement_org_id == ''){
        ...
        let varsett = this.isPaymentListPayEdit ? this.ruleForm.settlement_org_id : this.$route.query.id;
        let params = {
          data: {
    ...
    getMatchPeerAccount(query){
      if(this.isPaymentListPayEdit && this.ruleForm.settlement_org_id == ''){
        ...
        let varpeer = this.isPaymentListPayEdit ? this.ruleForm.settlement_org_id : this.$route.query.id;
    ...
    saveForm() {
      ...
      let varid = this.$route.name == 'paymentListPay-edit' ? this.ruleForm.settlement_org_id : this.$route.query.id,
        varBankName = '',
        varAccount = '',
        varAccountName = '';
      ...
      api[action - 1]({
        ...
        success: (params) => {
          ...
          if(data.code=='0'){
            ...
            setTimeout(function () {
              if(_this.$route.name == 'paymentListPay-edit'){
                // 付款处理-付款记录-添加登记付款 返回上一级
                _this.$router.push({ path: '/finace/paymentList', query: {} })
              }else{
                _this.$router.push({ path: '/fund-settlement/pay-record', query: { id: relate_id } })
              }
            }, 500);
          }else{
            this.$message.error(data.msg);
