      ...
      <div class="content-list">
        <div class="list">
          <el-table :data="tableData" border style="width: 100%">
            ...
            <el-table-column prop="rmb_total_amount" label="资金流向" align="right" width="100" :resizable="false">
              <template slot-scope="scope">
                <template v-if="parseFloat(scope.row.rmb_total_amount)>=0">
                  <span style="color:#019F3D;">￥ +{{scope.row.rmb_total_amount}}</span>
                </template>
                <template v-else>￥ {{scope.row.rmb_total_amount}}</template>
              </template>
            </el-table-column>
            <el-table-column prop="fund_balance" label="余额" align="right" width="100" :resizable="false">
            ...
            <el-table-column prop="settlement_org_name" label="结算方名称" align="center" min-width="120" :resizable="false">
            </el-table-column>
            ...
            <el-table-column label="收付款凭证" align="center" width="120" fixed="right" prop="voucher_name">
              ...
              <template slot-scope="scope">
                <template v-if="scope.row.voucher_type==1">
                  <router-link v-if="$menuPermission('api/zhike/customer_payment_list')" :to="{path: '/jdire-list', query: {id:scope.row.bill_id}}">{{scope.row.voucher_name}}</router-link>
                </template>
                <template v-if="scope.row.voucher_type==2">
                  <router-link v-if="$menuPermission('api/finance/get_payment_detail_list')" :to="{path: '/zrecieve-settlment-list', query: {id:scope.row.payment_id,bill_id:scope.row.bill_id}}">{{scope.row.voucher_name}}</router-link>
                  
                </template>
                <!-- <template v-if="scope.row.voucher_type==3">
                  <router-link v-if="$menuPermission('api/finance/payment_list')" :to="{path: '/fund-settlement/pay-record', query: {id:scope.row.settlement_org_id}}">{{scope.row.voucher_name}}</router-link>
                </template>
                <template v-if="scope.row.voucher_type==4">
                  <router-link v-if="$menuPermission('api/finance/get_payment_detail_list')" :to="{path: '/pay-settlment-list', query: {id:scope.row.payment_id}}">{{scope.row.voucher_name}}</router-link>
                </template> -->
              </template>
            </el-table-column>
          </el-table>
