      ...
      <div class="content-list">
        <div class="scenic-table-module" v-for="(scenic,idx) in tableData" :key="idx">
          <el-row class="top-level">
            <el-col :span="16">
                <span class="name" @click="$router.push({path:'/self-support/scenic/detail',query:{id:scenic.id}})">{{scenic.title}}</span> 
            ...
            <el-col :span="8">
              <div class="btn-tools">
                <el-button type="text" @click="$router.push({path:'/self-support/scenic/edit',query:{id:scenic.id}})">编辑景区</el-button>
                <el-button type="text" @click="initTicketForm(scenic)">新增门票</el-button>
                <el-button type="text" @click="changeScenicStatus(scenic)">{{scenic.status == 1?'禁用':'启用'}}</el-button>
              </div>
            </el-col>
          </el-row>
          ...
          <el-row class="sub-level">
            <el-col :span="16">
              <div>
                <span class="name" @click="ticketDetailShow(ticket.id,'detail')">{{ticket.name}}</span>
            ...
            <el-col :span="8">
              <div class="btn-tools">
                <el-button type="text" @click="thirdQuoteShowPannel(ticket)">第三报价</el-button>
                <el-button type="text" @click="$router.push({path:'/self-support/scenic/quote',query:{ticketId:ticket.id}})">新增报价</el-button>
                <el-button type="text" @click="ticketDetailShow(ticket.id,'form')">编辑门票</el-button>
                <el-button type="text" @click="changeTicketStatus(ticket)">{{ticket.status == 1?'禁用':'启用'}}</el-button>
              </div>
            </el-col>
          </el-row>
              ...
              <td>
                ...
                <el-dropdown>
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item @click.native="$router.push({path:'/self-support/scenic/quote-detail',
                      query:{quoteId:quote.id}})">详情</el-dropdown-item>
                    <el-dropdown-item @click.native="$router.push({path:'/self-support/scenic/quote',
                      query:{ticketId:quote.ticket_id,quoteId:quote.id}})" 
                      v-if="quote.sale_status != 2 &&
                      quote.sale_status != 3">
                      编辑
                    </el-dropdown-item>
                    <el-dropdown-item @click.native="changeQuoteOperaStatus(quote.id,1)" 
                      v-if="quote.sale_status == 1 || 
                      quote.sale_status == 4 ||
                      quote.sale_status == 5 ||
                      quote.sale_status == 6
                      ">上架</el-dropdown-item>
                    <el-dropdown-item @click.native="changeQuoteOperaStatus(quote.id,2)" 
                      v-if="quote.sale_status == 3">下架</el-dropdown-item>
                    ...
                    <el-dropdown-item @click.native="changeQuoteOperaStatus(quote.id,3)" 
                      v-if="quote.sale_status == 2">撤回</el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              </td>
            </tr>
    ...
    size="small">
      <el-form :model="ticketForm" label-width="100px" ref="ticketForm"
        ...
        <el-form-item label="门票名称" prop="name"
          :rules="[{ required: true, message: '请填写门票名称', trigger: 'blur' }]"
          >
...
import ZoneAutocomplete from './hotel/zone-autocomplete/index.vue';
