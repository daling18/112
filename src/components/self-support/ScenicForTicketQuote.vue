      ...
      <div class="mod-form" v-if="form">
        <el-form label-width="120px" style="width:80%;" ref="scenicForm" :model="form" :rules="rules">
          <el-form-item label="产品来源" prop="supplier_id">
            <select-supplier v-model="form.supplier_id" :orgName.sync="form.supplier_name"></select-supplier>
          ...
          <el-form-item v-if="form.use_type == 0">
            允许推迟 <el-input v-model="form.start_date" placeholder="" class="mini-form-input" /> 天
            <span>注：0代表不设置</span>
          </el-form-item> -->
        ...
        if(valid){
          ...
          setSelfSupportTicketQuote({
            ...
            success: (params) => {
              ...
              if(!code){
                this.$router.push({path:'/self-support/scenic/quote-detail',
