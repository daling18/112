...
export default {
  data(){
    ...
    let quote = ValidateFun('quote'),
        ...
        validateRefundDate = (rule, value, callback) => {
          if(this.packageForm.base.date_type == 2){
            if (value === '') {
        ...
        validateRefundMin = (rule, value, callback) => {
          if(this.packageForm.base.date_type == 2){
            if (value === '') {
    ...
    return {
      ...
      packageFormRules:{
        ...
        'base.date_day': [
          { validator: validateRefundDate, trigger: 'change'  }
        ],
        'base.date_min': [
          { validator: validateRefundMin, trigger: 'change'  }
        ]
      },
      quoteForm:{
        startDate: null,
  ...
  watch:{
    'packageForm.base.date_type': {
      handler(val){
        if(val == 1){
          this.$nextTick(() => {
            this.$refs.packageForm.validateField('base.date_day');
            this.$refs.packageForm.validateField('base.date_min');
          });
        } 
      }
    },
