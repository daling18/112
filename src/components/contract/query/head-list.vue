<template>
  <div class="wrapper">
    <div class="system-tab">
      <ul class="tab-box">
        <li class="on">总部合同</li>
        <li @click="goto('/contract/query/branch-list',{})" v-if="$menuPermission('api/contract/get_contract_no_list/cx/1')">分公司合同</li>
      </ul>
    </div>
    <!-- system-search -->
      ...
      <div class="content-list">
        <div class="list" v-if="tableData">
          <el-table :data="tableData" style="width: 100%">
            <el-table-column prop="contract_no" label="合同编号" align="left" min-width="250">
            ...
            <el-table-column label="操作" align="center" width="140">
              <template slot-scope="scope">
                <span v-for="(o,i) in scope.row.menu"
                      :key="i">
                  <!-- 1绑定订单 -->
                  <router-link v-if="o.id == 1 && $menuPermission('api/contract/bind_order/add/2')"
                               :to="{path:'/contract/'+(o.tag=='lq'?'pulldown':'audit')+'/'+typePre+'-border-a', query:{type:2,menu:(o.tag=='lq'?1:2),status:0,id:scope.row.contract_no}}">
                    {{ o.name }}
                  </router-link>
                  <!-- 3修改订单 -->
                  <router-link v-if="o.id == 3 && $menuPermission('api/contract/bind_order/edit/2')"
                               :to="{path:'/contract/'+(o.tag=='lq'?'pulldown':'audit')+'/'+typePre+'-border-e', query:{type:2,menu:(o.tag=='lq'?1:2),status:1,id:scope.row.contract_no}}">
                    {{ o.name }}
                  </router-link>
                  <!-- 2核销 -->
                  <router-link v-if="o.id == 2 && $menuPermission('api/contract/use_cancel_add/hx/2')"
                               :to="{path:'/contract/'+(o.tag=='lq'?'pulldown':'audit')+'/'+typePre+'-verify', query:{type:2,menu:(o.tag=='lq'?1:2),status:0,aid:scope.row.a_id,id:scope.row.contract_no}}">
                    {{ o.name }}
                  </router-link>
                  <!-- 5重新核销 -->
                  <router-link v-if="o.id == 5 && $menuPermission('api/contract/use_cancel_add/cxhx/2')"
                               :to="{path:'/contract/'+(o.tag=='lq'?'pulldown':'audit')+'/'+typePre+'-reverify', query:{type:2,menu:(o.tag=='lq'?1:2),status:1,aid:scope.row.a_id,id:scope.row.contract_no}}">
                    {{ o.name }}
                  </router-link>
                  <!-- 4查看核销记录 -->
                  <router-link v-if="o.id == 4 && $menuPermission('api/contract/get_use_cancel/jl/cx/2')"
                               :to="{path:'/contract/'+(o.tag=='lq'?'pulldown':'audit')+'/verify-record', query:{type:2,menu:(o.tag=='lq'?1:2),dtype:1,id:scope.row.contract_no}}">
                    {{ o.name }}
                  </router-link>
                  <!-- 6审核 -->
                  <router-link v-if="o.id == 6 && $menuPermission('api/contract/use_cancel_review/cx/2')"
                               :to="{path:'/contract/'+(o.tag=='lq'?'pulldown':'audit')+'/verify-record',query:{type:2,menu:(o.tag=='lq'?1:2),dtype:2,id:scope.row.contract_no}}">
                    {{ o.name }}
                  </router-link>
                </span>
...
<script>
import { getContractNumberList2 } from 'src/api/api';
