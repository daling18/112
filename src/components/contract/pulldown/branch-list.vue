<template>
  ...
  <div class="wrapper">
    ...
    <div class="system-tab">
      <ul class="tab-box">
        <li v-if="$menuPermission('api/contract/apply_list/2/'+roleMenuVar)">
            ...
            <el-table-column label="审批人"
              ...
              <template slot-scope="scope">
                <div class="ellipsis-clamp2 h40"
                     :title="scope.row | getApprover">
                  {{ scope.row | getApprover }}
                </div>
              </template>
            </el-table-column>
            ...
            <el-table-column label="操作"
              ...
              <template slot-scope="scope">
                <span v-for="(o,i) in scope.row.menu"
                      :key="i">
                  <router-link 
                               v-if="o.menu_name == '详情' && $menuPermission('api/contract/apply_detail/1/'+roleMenuVar+'/1')"
                               :to="{path:'/contract/'+rolePath+'/branch-detail',query:{type:1,menu:roleMenuVar,dtype:1,id:scope.row.id}}">
                    {{ o.menu_name }}
                  </router-link>
                  <router-link
                               v-if="o.menu_name == '审核' && $menuPermission('api/contract/apply_detail/1/2/2')"
                               :to="{path:'/contract/'+rolePath+'/branch-detail',query:{type:1,menu:roleMenuVar,dtype:2,id:scope.row.id}}">
                    {{ o.menu_name }}
                  </router-link>
                  <router-link
                               v-if="o.menu_name == '合同编号' && $menuPermission('api/contract/get_contract_no_list/'+roleMenuBEName+'/1')"
                               :to="{path:'/contract/'+rolePath+'/branch-number',query:{type:1,menu:roleMenuVar,id:scope.row.id}}">
                    {{ o.menu_name }}
                  </router-link>
                </span>
...
<script>
import {
  getContractPulldownList1,
  getContractAuditList1,
...
export default{
  filters: {
    getApprover(row){
      return row.to_org_name + ' / ' + row.to_user_name;
    }
  },
  data(){
    return{
  ...
  created(){
    this.$route.name == 'contractPulldownBranchList' ? this.role = 'pulldown' : this.role = 'audit';
    if(this.role == 'pulldown'){
