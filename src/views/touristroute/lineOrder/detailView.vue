      ...
      lineQuoteOrdeList({
        ...
        if (code == 0) {
          for (let i in data.list) {
            this.navOptions = [{
              label: "订单详情",
              path: `/touristroute/v2/lineOrder/detail/order?order_id=${ data.list[i].id }&order_no=${ data.list[i].order_no }`
            }]
            for (let l in data.list[i].lineRefund) {
              if (data.list[i].lineRefund[l] && data.list[i].lineRefund[l].order_no && data.list[i].lineRefund[l].cancel_no) {
                this.navOptions.push({
                  label: "退款详情",
                  path: `/touristroute/v2/lineOrder/detail/refund?order_no=${ data.list[i].lineRefund[l].order_no }&cancel_no=${ data.list[i].lineRefund[l].cancel_no }`
                })
              }
            }
            break;
