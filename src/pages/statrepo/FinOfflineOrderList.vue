<!--D:\files\repo\bom\application\index\view\dev\src\components\finace\report\index2.vue-->
<template>
	<div class="wrapper">
			...
			<div class="report-hd clearfix">
				<div class="tab clearfix">
					<router-link class="li"
						v-if="$menuPermission('statrepo/finarepo/offline')"
						:to="{path:urlPath+'tuanList', query:{id:id}}">
						团维度
					</router-link>
				...
				<div class="fr">
					<template v-if="sourceFrom == 1">
						{{allData.org_name}}
						<a :href="exportFunc" class="btn" v-if="$menuPermission('statrepo/comprepo/offlineorderexpo')">导出</a>
					</template>
					<template v-else>
						<a :href="exportFunc" class="btn" v-if="$menuPermission('statrepo/finarepo/offlineorderexpo')">导出</a>
					</template>
				</div>
			</div>
			<!-- /report-hd -->
						...
						<template slot-scope="scope">
							...
							<template v-else>
								...
								<template v-if="scope.row.contract_confirmation_type == 1">
									<router-link
										:to="{path: '/tuancontro/income/confirm-file', query: { parent_id:scope.row.tuan_id, id:scope.row.order_id }}">
										查看确认件
									</router-link>
								</template>
								<!-- 纸质合同 -->
								<template v-if="scope.row.contract_confirmation_type == 2">
									...
									<router-link
										:to="{path: '/saleorder/line/pager-contract', query: {'order_no':scope.row.order_no, 'tuan_id':scope.row.tuan_id, 'edit':'3'}}">
										查看纸质合同
									</router-link>
								</template>
								<!-- 电子合同展示合同编号 -->
...
<script>
import {dlElectronicContract} from 'src/api/tuan';
...
export default {
	...
	created(){
		if(this.$route.meta.menu == 'StatrepoFinarepo'){
			this.sourceFrom = 0;
			this.urlPath = '/statrepo/fin/offline/';
		}else{
			this.sourceFrom = 1;
			this.urlPath = '/statrepo/comp/offline/';
		}
	},
	components:{
		'long-tab': longTab
