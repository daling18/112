<!--D:\files\repo\bom\application\index\view\dev\src\components\finace\report\index.vue-->
<template>
	<div class="wrapper">
			...
			<div class="report-hd clearfix">
				<div class="tab clearfix">
					...
					<router-link class="li"
						v-if="$menuPermission('statrepo/finarepo/offlineorder')"
						:to="{path:urlPath+'orderList', query:{id:id}}">
						订单维度
					</router-link>
				</div>
				<div class="fr">
					<template v-if="sourceFrom == 1">
						{{allData.org_name}}
						<a :href="exportFunc" class="btn" v-if="$menuPermission('statrepo/comprepo/offlineexp')">导出</a>
					</template>
					<template v-else>
						<a :href="exportFunc" class="btn" v-if="$menuPermission('statrepo/finarepo/offlineexp')">导出</a>
					</template>
				</div>
			</div>
			<!-- /report-hd -->
