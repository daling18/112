    ...
    <div class="hotel">
      <div v-for="(hotel, key) in hotels" :key="key" class="hotel-item">
        ...
        <div class="hotel-item-other">
           ...
           <div v-if="$menuPermission('salecenter/hotel')" class="hotel-item-buybtn">
             <el-button type="primary" @click="$router.push({ name: 'SalecenterHotelDetail', query: { hotelId: hotel.hotel_code, time: +new Date(filters.time[0]) + '|' + +new Date(filters.time[1]) } })">预订</el-button>
           </div>
        </div>
      </div>
    </div>
