// src\components\distribution\LineListOrder.vue
<template>
  <div class="wrapper" v-loading="touistloading">
    <div class="order-header">
      ...
      <div class="header-content">
        <label class="color-gray">出发城市</label>
        {{ orderInfo.departure_city }}
        <br>
        <label class="color-gray">出发日期</label>
        <span class="w265">{{ orderInfo.departure_date }}</span>
        <label class="color-gray">返回日期</label>
        {{ orderInfo.back_date }}
      </div>
    </div>
    ...
    <div class="content-left fl bg-white">
      <div class="settle-section plr15">
        ...
        <div class="module-content">
          ...
          <div class="field-item">
            <div class="field-label">
              <span class="color-red">*</span>联系人姓名
            </div>
            <div class="field-content">
        ...
        <div class="module-title clear">
          ...
          <div class="fr export-excel-section">
            ...
            <el-upload
              ...
              action="/api/v1/order/tuan/import-tourists"
              :data="uploadData"
              name="file"
        ...
        <div class="module-content">
          <div class="tourist-item" v-for="(tourist, idx) in touristList" :key="tourist.id">
            <line-tourist-form
              ...
              :room-diff-price="room_diff_price"
      ...
      <div class="submit-section">
        <div class="total">
          ...
          <span class="color-orange font24">{{ isZhiKe || isStore ? salePrice : settlePrice }}</span>
        </div>
    ...
    <div class="content-right fl">
      ...
      <div class="price-list bg-white">
        <div class="price-item">
          ...
          <b>结算总价 {{ settlePrice }}</b>
          <!-- <b v-if="!isStore">结算总价 {{ settlePrice }}</b> -->
        </div>
        <div class="price-item" v-for="(detail, idx) in touristDetailList" :key="'tourist_' + idx">
          {{ detail.touristerType }} + {{detail.price_city_name}} + {{detail.price_package_name}} * {{ detail.num }} ({{ isZhiKe || isStore ? detail.sale_price : detail.settle_price }}) =
          <span
            class="color-orange"
          >{{ isZhiKe || isStore ? detail.sale_price * detail.num : detail.settle_price * detail.num }}</span>
        </div>
        <div class="price-item" v-if="orderInfo.room_diff_price && show_room_diff_price_roomnum != 0">
          单房差 * {{ show_room_diff_price_roomnum }} =
          <span
            class="color-orange"
          >{{ orderInfo.room_diff_price * show_room_diff_price_roomnum}}</span>
        </div>
        <div class="price-item" v-for="(detail, idx) in priceDetailList" :key="'price_' + idx">
          {{ detail.item_name || ('项目' + (idx + 1)) }} * {{ detail.num }} ({{ detail.operator == '-' && detail.unit_price ? '-' : '' }}{{ detail.unit_price || 0 }}) =
          <span class="color-orange">{{ detail.totalPrice }}</span>
        </div>
      </div>
      <!-- <div class="add-price-btn tac bg-white color-purple" v-if="!isStore" @click="handleAddPriceDetail()">+ 添加价格明细</div> -->
      <div class="add-price-btn tac bg-white color-purple" @click="handleAddPriceDetail()">+ 添加价格明细</div>
      <order-price-item
        class="bg-white"
...
import { promise_saleAddOrder, promise_zkAddOrder } from "src/api/promise_api";
...
import selectNeworg from "src/plugins/SelectNeworg.vue";
import InputNumber from "src/plugins/InputNumber.vue";
import OrderPriceItem from "src/components/components/OrderPriceItem.vue";
import LineTouristForm from "src/components/components/LineTouristForm.vue";
...
export default {
  ...
  data() {
    ...
    return {
      ...
      room_diff_price: 0,
      show_room_diff_price_roomnum: 0,
    };
  },
  computed: {
    isBOMTrip() {
    ...
    isZhiKe() {
      return (
        this.$route.name == "SalecenterLineZhikeOrder" ||
        this.$route.name == "SalecenterOtherLineZhikeOrder" || 
        this.$route.name == "SalecenterOperatingZhikeOrder" || 
        this.$route.name == "SalecenterOtherOperatingZhikeOrder"
      );
    },
    topMenuConst() {
    ...
    isStore() {
      ...
      return (
        !this.isZhiKe &&
        this.orderFormData.distributor_org_id == 0 &&
        this.orderFormData.distributor_org_name
      );
    },
    getOrderInfoFunc() {
      switch (this.topMenuConst) {
        case "salecenter":
          return this.isZhiKe ? promise_zkAddOrder : promise_saleAddOrder;
        // case 'operator':
    ...
    salePrice() {
      ...
      this.touristList.forEach(item => {
        if(item.is_select_room){
          price += (Number(item.sale_price) + Number(this.room_diff_price));
        }else{
    ...
    settlePrice() {
      ...
      this.touristList.forEach(item => {
        if(item.is_select_room){
          price += (Number(item.settle_price) + Number(this.room_diff_price));
        }else{
          price += Number(item.settle_price)
        }
      });
      // this.priceDetailList.forEach(item => {
      //   price += item.totalPrice;
  ...
  methods: {
    ...
    getInitTourist() {
      ...
      tourist.packages = this.EXTRA_INFO[0].package;
      tourist.touristerList = this.EXTRA_INFO[0].package[0].price;
      tourist.price_city_id = this.EXTRA_INFO[0].id;
      tourist.price_city_name = this.EXTRA_INFO[0].name;
      tourist.price_package_id = this.EXTRA_INFO[0].package[0].id;
      tourist.price_package_name = this.EXTRA_INFO[0].package[0].name;
      tourist.price_id = this.EXTRA_INFO[0].package[0].price[0].id;
      tourist.touristerType = this.EXTRA_INFO[0].package[0].price[0].name;
      tourist.sale_price = this.EXTRA_INFO[0].package[0].price[0].sales_price;
      tourist.settle_price = this.EXTRA_INFO[0].package[0].price[0].settle_price
      tourist.id = id;
      // tourist.price_city_id = tourist.price_city_id || "";
      // tourist.price_city_name = tourist.price_city_name || "";
      // tourist.price_package_id = tourist.price_package_id || "";
      // tourist.price_package_name = tourist.price_package_name || "";
      // tourist.price_id = tourist.price_id || "";
      // tourist.touristerType = tourist.touristerType || "";
      // tourist.settle_price = tourist.settle_price || "";
      // tourist.sale_price = tourist.sale_price || "";
      // tourist.num = tourist.num || 1;
      // tourist.is_select_room = tourist.is_select_room || "";
    ...
    getOrderInfo() {
      ...
      //   showLoading: true
      // });
      this._promise_saleAddOrder();
    },
    async _promise_saleAddOrder() {
      try {
        const params = {
          data: {
            tuan_id: this.tuanId
        ...
        const { data } = await this.getOrderInfoFunc(params);
        if (data.code !== 0) {
          this.$message({ message: data.msg, type: "error" });
          this.handleBack();
          return;
        }
        this.EXTRA_INFO = data.data.prices;
        const NEXTRA_INFO = data.data.prices;
        this.orderInfo = data.data;
        this.room_diff_price = data.data.room_diff_price;
        if (Array.isArray(data.data.org_info)) {
          this.DEFAULT_ORG_LIST = data.data.org_info;
        } else {
          this.orderFormData.distributor_org_name = data.data.org_info;
        }
    ...
    genTouristPriceDetail(tourist) {
      ...
      return Object.assign(detail,{...tourist})
    },
    // genTouristPriceDetail(tourist){
    //   const {
    //     orderInfo: {price_info: { out_price, peer_price }},
    ...
    //       const extra = EXTRA_MAP[it.price_city_id][it.id];
    //       const outPrice = extra.diff_out_price;
    //       const peerPrice = extra.diff_peer_price;
    ...
    redirectToDetail(data) {
      switch (this.topMenuConst) {
        ...
        //   break;
        case "salecenter":
          const prefixPath = this.isStore
            ? "/purchaseorder/line/" 
            : "/saleorder/line/";
          this.$router.push({
            path: `${prefixPath}order-detail/${data.id}/${data.order_no}`,
            query: { id: data.id }
          });
          break;
        default:
    ...
    handleUploadSuccess(resp, file) {
      ...
      if (resp.code == 0 && resp.data) {
        ...
        // this.addTouistDialogVisible = true
        this.$confirm("导入操作会覆盖已经填写的游客信息，是否继续？", "提示", {
          ...
          .then(() => {
            let id = 1;
            const { data } = resp.data;
            this.touistloading = true
            this.touristDetailList = [];
            // const touristLen = this.touristList.length;
            this.touristList = new Set();
            // const initTourist = LineTouristForm.getInitTourist();
            const initTourist = this.getInitTourist();
            ...
            if (Array.isArray(data)) {
              ...
              let that = this;
              setTimeout(function(){
                that.touristList = arr;
                that.handleCalcTouristDetail();
                that.touistloading = false;
              },500)
            }
          })
          .catch(() => {
    ...
    handleCalcTouristDetail() {
      const { touristList, EXTRA_MAP, isZhiKe, isStore } = this;
      let touristDetailList = new Set();
      let num = 0;
      for(let i=0; i<touristList.length;i++){
        let tourist = {...touristList[i]};
        let detail = this.genTouristPriceDetail(tourist);
        // let detail = tourist;
        let isHasItem = false;
        const price = isZhiKe || isStore ? detail.sale_price : detail.settle_price;
        if(detail.is_select_room){
          num++;
        }

        if(touristDetailList.length > 0){
          touristDetailList.forEach(item=>{
            if(item.price_city_id == detail.price_city_id && item.price_package_id == detail.price_package_id && item.price_id == detail.price_id){
              isHasItem = true;
              item.num++;
            }
          })
      ...
      this.show_room_diff_price_roomnum = num;
      this.touristDetailList = touristDetailList;
    },
    handleAddTourist() {

      let tourist = this.getInitTourist();
      tourist.price_city_id = tourist.price_city_id || "";
      tourist.price_city_name = tourist.price_city_name || "";
      tourist.price_package_id = tourist.price_package_id || "";
      tourist.price_package_name = tourist.price_package_name || "";
    ...
    handleDelTourist(idx) {
      ...
      this.handleCalcTouristDetail();
      // const id = tourist.map(it => `${it.price_city_id}.${it.id}`).join("_");
      // let touristDetail = "";
      // let detailIndex = -1;
      ...
      //     touristDetail.totalOutPrice = parseFloat(
      //       touristDetail.totalOutPrice.toFixed(2)
      //     );
      //     touristDetail.totalPeerPrice = parseFloat(
      //       touristDetail.totalPeerPrice.toFixed(2)
      //     );
      //   } else {
    ...
    handleSubmitForm() {
      ...
      if (!orderFormData.distributor_mobile) {
        return this.$message.error("请填写联系电话");
      }
      if (!reg.test(orderFormData.distributor_mobile)) {
        return this.$message.error("请填写正确的电话号码");
      }

      // 格式化日历插件的时间，日历插件默认返回Date类型时间
      let orderTouristList = [...touristList];
      orderTouristList.forEach(item => {
        item.identity_end_date = item.identity_end_date
          ? format.date(item.identity_end_date)
          : "";
        item.birthday = item.birthday ? format.date(item.birthday) : "";
        allTouristIds.push(item.id);
        // delete item.packages;
        // delete item.touristerList;
      });
      // this.checkTouristList();
      orderFormData.tourist_lists = orderTouristList;
      ...
      this.setOrderInfoFunc({
        ...
        success: res => {
          ...
          if (res.data.code == "0") {
            this.redirectToDetail(res.data.data);
          }
        },
        showLoading: true
      });
