    ...
    <div class="package" id="package">
      <div class="package-hd">
        <div class="hotel-info">
          <strong>酒店名称：<router-link :to="{ name: 'SingleproHotelEdit', query: { id: hotelId } }">{{ hotelName }}</router-link></strong>
          <strong>房型名称：<a href="javascript:;" @click="handleToEditRoom">{{ roomName }}</a></strong>
        </div>
        <div v-if="$menuPermission('singlepro/hotel/aeroompac')" class="tools">
            ...
            <template slot-scope="prop">
              <el-dropdown>
                ...
                <el-dropdown-menu slot="dropdown">
                  <el-dropdown-item v-if="$menuPermission('singlepro/hotel/roompacdet')">
                    <a href="javascript:;" @click="handleDetailRoomPackage(prop.row)">详情</a>
                  </el-dropdown-item>
                  <el-dropdown-item v-if="$menuPermission('singlepro/hotel/aeroompac') && prop.row.release != 3 && prop.row.source != 4">
                    <a href="javascript:;" @click="handleEditRoomPackage(prop.row)">编辑</a>
                  </el-dropdown-item>
                  <template v-if="$menuPermission('singlepro/hotel/chgroompacstatus')">
                  ...
                  <template v-if="$menuPermission('singlepro/hotel/chgroompacstatus/chghotelstatus')">
                    <template v-if="prop.row.status == 1">
