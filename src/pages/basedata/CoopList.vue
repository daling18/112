<template>
  <!-- application\index\view\dev\src\components\system\supplier\CoopList.vue -->
  <div class="wrapper supplier-wrap">
    <div class="system-tab">
      <ul class="tab-box">
        <router-link tag="li"
          to="/basedata/supplier"
          v-if="$menuPermission('basedata/supplier/manage')">
          供应商管理
        </router-link>
        <router-link tag="li"
          class="on"
      ...
      <div class="content-list">
        <el-table
          ...
          <el-table-column
            ...
            min-width='200'>
            <template slot-scope="scope">
              <div class="ellipsis" :title="scope.row.first_cooperation_name">
                {{scope.row.first_cooperation_name || '---'}}
              </div>
            </template>
          </el-table-column>
          <el-table-column
          ...
          <el-table-column
            ...
            width="180">
            <template slot-scope="scope">
              <div v-if='$menuPermission("basedata/supplier/confirmcooperation/supplierdetail")'>
                <router-link
                  v-for="(o,i) in scope.row.edit"
                  :key="i"
                  :to="{path: 'coop-list/detail', query:{id:scope.row.id, type:2}}">
                  {{allData.edit_arr[o]}}
                </router-link>
              </div>
              <div v-else>--</div>
