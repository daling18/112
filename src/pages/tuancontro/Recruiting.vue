<template>
<!-- application\index\view\dev\src\components\plan\control\recruiting.vue -->
  <div id="pdf-wrap">
    ...
    <div class="recruiting" v-show="step=='tourist-list'">
      <div class="gird">
        <div class="filter no-print">
          ...
          <a :href="printName" class="el-button el-button--primary fr" v-if="tuanInfo && tuanInfo.is_self === 1 && $menuPermission('tuancontro/line/touristlist/outtour')">导出名单</a>
        </div>
        <div class="cont">
          <el-table
            ref="touristTable"
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <component
                  :is="$menuPermission('touristroute/lineorder/lineorder/getbackdetail') ? 'router-link' : 'span'"
...
export default {
  ...
  computed:{
    ...
    canMoveTuan() {
      const isOperator = this.topMenuConst == 'tuancontro'
      ...
      return isOperator && this.tableData.length && this.productType == 1 && isNotLock && this.$menuPermission('tuancontro/line/touristlist/tuantour')
    }
  },
  watch:{
      ...
      doCreateOrder({
        ...
        success: (params) => {
          ...
          if(code == 0){
            ...
            // if(this.$menuPermission('tuancontro/line/tuanacc')) {
            if (this.$menuPermission('touristroute/lineorder/lineorder')) {
              // this.$router.push({path: '/tuancontro/tuan-list', query: {id: this.selectTuanInfo.a_id, lock_status: 0}});
              this.$router.push({path: '/touristroute/order-list'})
            } else {
              this.showMove = false;
