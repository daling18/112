<template>
	<div class="wrapper">
		<!-- application\index\view\dev\src\components\plan\cost\list.vue -->
		<div class="system-content" v-if="costData.tuan_info">
			...
			<div class="content-title">
				...
				<el-button
					class="fr"
					v-if="!multipleCheckType && costPermission.addPayment"
                  ...
                  <span v-if="scope.row.invoice_flag_name">[{{scope.row.invoice_flag_name}}]</span>￥{{scope.row.rmb_total_amount}}
						...
						<el-table-column
							...
							<template slot-scope="scope">
								<div class="color-orange">
									<router-link 
										v-if="$menuPermission('settlement/pay/payapplist/getpayappdet') && scope.row.is_add_applied === 0"
										:to="{path: '/settlement/pay/apply-detail', query: {id: scope.row.last_applied_id}}">
										[申]
									</router-link>
									<el-popover
										placement="bottom"
						...
						<el-table-column
							...
							<template slot-scope="scope">
								<el-dropdown 
									v-if="costPermission.costAdd
										|| costPermission.costDel
										|| costPermission.updateSure
										|| costPermission.sureDetail
										|| costPermission.invoiceAdd
										|| costPermission.invoiceDtl
										|| costPermission.costDtl
										|| costPermission.addPayment
										|| costPermission.applyList"
									trigger="click">
									...
									<el-dropdown-menu slot="dropdown">
										<el-dropdown-item v-if="costPermission.costAdd && scope.row.item_type != 14 && scope.row.order_no == '--'">
											<!-- <a href="javascript:;" @click="costModify(scope)">编辑</a> -->
											<router-link :to="{ name: costAddRouterName, query: {id:scope.row.id,tuan_id: costData.tuan_info.resource_id}}">
												编辑
											</router-link>
										</el-dropdown-item>
										<el-dropdown-item v-if="costPermission.costDel && scope.row.order_no == '--'">
											<a href="javascript:;" @click="costDelete(scope)">删除</a>
										</el-dropdown-item>
										<el-dropdown-item v-if="costPermission.updateSure && costData.tuan_info.type != 4">
											<router-link :to="{ path: '/tuancontro/cost/confirm', query: {parent_id: costData.tuan_info.resource_id, id:scope.row.id, type:2}}">
												上传确认件
											</router-link>
										</el-dropdown-item>
										<el-dropdown-item v-if="costPermission.sureDetail && costData.tuan_info.type != 4">
											<router-link :to="{ path: '/tuancontro/cost/comfirmDownloadPdf', query: { parent_id: costData.tuan_info.resource_id, id:scope.row.id }}">
												确认件模版
											</router-link>
										</el-dropdown-item>
										<el-dropdown-item v-if="costPermission.invoiceAdd && scope.row.invoice_flag == 0">
											<router-link class="span-btn"
												:to="{path: '/settlement/invoice/cost/add', query: {id: scope.row.id, type: 1}}">
												上传成本票
											</router-link>
										</el-dropdown-item>
										<el-dropdown-item v-if="costPermission.invoiceDtl && scope.row.invoice_detail_flag == 1">
											<router-link class="span-btn"
												:to="{path: '/settlement/invoice/cost/detail', query: {bid: scope.row.id, type: 1}}">
												查看成本票
											</router-link>
										</el-dropdown-item>
										<el-dropdown-item v-if="costPermission.costDtl">
											<router-link class="span-btn"	:to="{name: costDetailRouterName, query: {aid:costData.tuan_info.a_id, id:scope.row.id}}">
												查看成本
											</router-link>
										</el-dropdown-item>
										<el-dropdown-item v-if="costPermission.addPayment && scope.row.is_add_applied === 1">
											<router-link class="span-btn"	:to="{path: '/tuancontro/applied-add', query: {ids: scope.row.id}}">
												申请付款
											</router-link>
										</el-dropdown-item>
										<el-dropdown-item v-if="costPermission.applyList && scope.row.is_add_applied === 0">
											<router-link class="span-btn"	:to="{path: '/settlement/pay/apply-list', query: {bill_id: scope.row.id}}">
												查看付款申请
											</router-link>
										</el-dropdown-item>
									</el-dropdown-menu>
  ...
  filters: {
	...
	computed: {
		...
		costAddRouterName(){
			...
			switch(type){
				case 'line':
					...
					break;
				case 'single':
					routeName = 'TuancontroSingleCostAdd';
					break;
			}
			return routeName
		},
		costDetailRouterName(){
			let type = this.costType,
					routeName = '';
			switch(type){
				case 'line':
					routeName = 'PlanCostDetail';
					break;
				case 'single':
					routeName = 'TuancontroSinglePlanCostDetail';
					break;
			}
			return routeName
		},
		costPermission(){
			...
			switch(type){
				case 'line':
					permission.costAdd = this.$menuPermission('tuancontro/line/costadd');
					permission.costDel = this.$menuPermission('tuancontro/line/costlist/delete');
					permission.costDtl = this.$menuPermission('tuancontro/line/costlist/detail');
					permission.updateSure = this.$menuPermission('tuancontro/line/costlist/updateSure');
					permission.sureDetail = this.$menuPermission('tuancontro/line/costlist/suredetail');
					permission.invoiceAdd = this.$menuPermission('settlement/invoice/cost/edit');
					permission.invoiceDtl = this.$menuPermission('settlement/invoice/cost/detail');
					permission.addPayment = this.$menuPermission('tuancontro/line/costlist/addpayment');
					permission.applyList = this.$menuPermission('settlement/pay/applylist');
					break;
				case 'single':
					permission.costAdd = this.$menuPermission('tuancontro/single/costlist/aecost');
					permission.costDel = this.$menuPermission('tuancontro/single/costlist/delcost');
					permission.costDtl = this.$menuPermission('tuancontro/single/costlist/detail');
					break;
			}
			return permission
		},
