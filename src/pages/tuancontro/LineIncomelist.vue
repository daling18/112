      ...
      <div class="search-content">
        <div class="clearfix">
          ...
          <el-col :span="16" class="text-right">
            <el-button 
              v-if="incomePermission.incomeEdit" 
              type="primary" 
              size="long" 
            ...
            >+ 添加收入应收款</el-button>
            <router-link 
              v-if="incomePermission.addRecei" 
              class="el-button el-button--primary" 
              :to="{
                path:'/settlement/recpay/receive-edit',
                query:{parent_id: imComeData.tuan_info && imComeData.tuan_info.id, action: 1, from:'tuancontro'}}"
            >+ 添加收款</router-link>
          </el-col>
        </div>
        <div style="padding-top: 10px;">
          ...
          <el-select placeholder="请选择" v-model="filters.bill_type" style="width:150px;">
            ...
            <el-option
              v-for="(obj,key) in imComeData.bill_type_lists"
              :key="key"
              :label="obj"
              :value="Number(key)"
            />
          ...
          <template v-if="imComeData.tuan_info">
            <template v-if="imComeData.tuan_info.type != 4">
              ...
              <el-select placeholder="请选择" v-model="filters.order_status" style="width:150px;">
                <el-option label="全部" value="0"/>
                <el-option
                  v-for="(obj,key) in imComeData.order_status_lists"
                  :key="key"
                  :label="obj"
                  :value="Number(key)"
                />
          ...
          <el-select placeholder="请选择" v-model="filters.invoice_flag" style="width:150px;">
            <el-option label="全部" value="0"/>
            <el-option
              v-for="(obj,key) in imComeData.invoice_flag_list"
              :key="key"
              :label="obj"
              :value="Number(key)"
            />
          </el-select>
          <div v-if="multipleCheckType && incomePermission.pressBill" style="display: inline-block; padding-left: 10px;">
          ...
          <el-button
            v-show="!multipleCheckType && incomePermission.pressBill"
            type="text"
            class="color-purple"
              ...
              <template slot-scope="scope">
                ...
                <el-dropdown trigger="click" v-else>
                  ...
                  <el-dropdown-menu slot="dropdown">
                    <el-dropdown-item v-if="isShowOperate(1, scope.row)" @click.native="incomeModify(scope)">编辑</el-dropdown-item>
                    <el-dropdown-item v-if="isShowOperate(2, scope.row)" @click.native="incomeDelete(scope)">删除</el-dropdown-item>
                    ...
                    <!-- </el-dropdown-item> -->
                    <template v-if="incomePermission.incomeDtl && imComeData.tuan_info.type != 4">
                      <!-- <el-dropdown-item
                      ...
                      <!-- 新的确认件链接 is_new=1 -->
                      <el-dropdown-item
                      v-if="isShowOperate(5, scope.row)">
                        <router-link :to="{path: 'income/confirm-file', query: { parent_id: imComeData.tuan_info.id, id:scope.row.order_id }}">确认件</router-link>
                      </el-dropdown-item>
                    </template>
                    <el-dropdown-item v-if="isShowOperate(6, scope.row)" @click.native="addShoukuan(scope)">添加收款</el-dropdown-item>
                    <el-dropdown-item>
                      <!-- oid的值来源，其他部分是取order_no，仅此处是bill_id -->
                      <router-link
                        :to="{ name: incomeDetailRouterName, query: {aid:imComeData.tuan_info.a_id, bid:scope.row.id}}"
                        v-if="isShowOperate(7, scope.row)"
                      >查看收入</router-link>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="isShowOperate(8, scope.row)">
                      <router-link :to="{path: '/settlement/invoice/income/add', query: {id: scope.row.id, type: 1}}" class="span-btn">申请开票</router-link>
                    </el-dropdown-item>
                    <el-dropdown-item v-if="isShowOperate(9, scope.row)">
                      <router-link :to="{path: '/settlement/invoice/income/detail', query: {bid: scope.row.id, type: 1}}" class="span-btn">查看开票详情</router-link>
                    </el-dropdown-item>
                  </el-dropdown-menu>
                </el-dropdown>
              </template>
    ...
    <el-dialog title="催款" :visible.sync="pressDlg">
      <el-form
        ...
        <el-form-item
          ...
          label="催款说明"
          class="mb0 oh">
          <el-input v-model="pressBillData.remark" type="textarea"/>
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
...
export default {
  ...
  computed: {
    incomeType(){
			...
			switch(routeName){
				case 'Income':
					type = 'line';
					break;
				case 'TuancontroSingleIncome':
					type = 'single';
					break;
			}
			return type
    },
    incomeDetailRouterName(){
			let type = this.incomeType,
					routeName = '';
			switch(type){
				case 'line':
					routeName = 'ControlIncomeIncomeDetail';
					break;
				case 'single':
					routeName = 'TuancontroSingleIncomeDetail';
					break;
			}
			return routeName
		},
    incomePermission(){
			...
			switch(type){
				case 'line':
					permission.incomeEdit = this.$menuPermission('tuancontro/line/incomeadd');
					permission.incomeDel = this.$menuPermission('tuancontro/line/incomelist/delete');
          permission.incomeDtl = this.$menuPermission('tuancontro/line/incomelist/getincodet');
          permission.addRecei = this.$menuPermission('settlement/recpay/recei/addrecei');
					permission.invoiceEdit = this.$menuPermission('settlement/invoice/income/edit');
          permission.invoiceDtl = this.$menuPermission('settlement/invoice/income/detail');
          permission.pressBill = this.$menuPermission('tuancontro/line/incomelist/pressbill')
					break;
				case 'single':
					permission.incomeEdit = this.$menuPermission('tuancontro/single/incomelist/aeincome');
					permission.incomeDel = this.$menuPermission('tuancontro/single/incomelist/delincome');
          permission.incomeDtl = this.$menuPermission('tuancontro/single/incomelist/getincodet');
					break;
      }
			return permission
		},
  ...
  methods: {
    ...
    isShowOperate(edit, income) {
      if (!edit) {
        return [1,2,3,4,5,6,7,8,9].some(e => this.isShowOperate(e, income))
      }
      switch (edit) {
      case 1:  // '编辑'
        return this.incomePermission.incomeEdit && income.bill_type !== 1 && income.order_no == '--';
      case 2:   // '删除'
        return this.incomePermission.incomeDel && income.order_no == '--';
      case 3:   // '确认件1'
        return this.incomeType == 'line' && income.order_from == 1 && income.order_no != '--' && income.confirm_is_new == 0;
      case 4:   // '确认件2', 
        return this.incomeType == 'line' &&income.order_from == 2 && income.order_no != '--' && income.confirm_file;
      case 5:   // '确认件3', 
        return this.incomeType == 'line' &&income.order_from == 1 && income.order_no != '--' && income.confirm_is_new == 1;
      case 6:   // '添加收款'
        return this.incomePermission.addRecei;
      case 7:   // '查看收入'
        return this.incomePermission.incomeDtl;
      case 8:   // '申请开票'
        return this.incomePermission.invoiceEdit && income.invoice_flag == 0
      case 9:   // '查看开票详情'
        return this.incomePermission.invoiceDtl && income.invoice_detail_flag == 1
      default:
        return false;
      }
    },
