<!--D:\files\repo\bom\application\index\view\dev\src\components\finace\balance\payDetail.vue-->
<template>
  <div class="recieve-info">
    <h2 class="recieve-tit">付款信息</h2>
    <div class="receive-content">
      <div class="content-box">
        <span class="content-label">付款金额</span>
        <span v-if="payment_info && payment_info.rmb_total_amount" class="content-right">
          <span class="color-orange">
            {{payment_info.rmb_total_amount}}
          </span>
          <span v-if="payment_info.payment_status_text === '审批通过'" class="color-green">[{{payment_info.payment_status_text}}]</span>
          <span v-else class="color-red">[{{payment_info.payment_status_text}}]</span>
        </span>
        <span v-else class="content-right">--</span>
      </div>
      <div class="content-box">
        <span class="content-label">付款ID</span>
        <span v-if="payment_info && payment_info.id" class="content-right">
          {{payment_info.id}}
        </span>
        <span v-else class="content-right">--</span>
      </div>
      <div class="content-box">
        ...
        <span class="content-right">
          <template v-if="payment_info && payment_info.settlement_org_name">
            {{payment_info.settlement_org_name}}
          </template>
          <template v-else>
            --
          </template>
      ...
      <!-- 收款人 -->
      <div class="content-box">
        <span class="content-label">收款人名称</span>
        <span v-if="payment_info && payment_info.receipt_account_name_info" class="content-right">
          {{payment_info.receipt_account_name_info}}
        </span>
        <span v-else class="content-right">--</span>
      </div>
      <div class="content-box">
        <span class="content-label">收款人账户</span>
        <span v-if="payment_info && payment_info.receipt_account_info" class="content-right">
          {{payment_info.receipt_account_info}}
        </span>
        <span v-else class="content-right">--</span>
      </div>
      <div class="content-box">
        <span class="content-label">开户行名称</span>
        <span v-if="payment_info && payment_info.receipt_bank_name_info" class="content-right">
          {{payment_info.receipt_bank_name_info}}
        </span>
        <span v-else class="content-right">--</span>
      </div>
      <!-- /收款人 -->
      <div class="content-box">
        <span class="content-label">付款账户</span>
        <span v-if="payment_info || payment_info.receipt_bank_name" class="content-right">
          {{payment_info.receipt_bank_name}} <span v-if="payment_info && payment_info.receipt_bank_name">|</span> {{payment_info.receipt_account}}
        </span>
        <span v-else class="content-right">--</span>
      </div>
      <div class="content-box">
        <span class="content-label">付款日期</span>
        <span v-if="payment_info && payment_info.create_time" class="content-right">
          {{payment_info.create_time}}
        </span>
        <span v-else class="content-right">--</span>
      </div>
      <div class="content-box">
        <span class="content-label">处理时间</span>
        <span v-if="payment_info && payment_info.update_time" class="content-right">
          {{payment_info.update_time}}
        </span>
        <span v-else class="content-right">--</span>
      </div>
      <div class="content-box">
        <span class="content-label">付款操作人</span>
        <span v-if="payment_info && payment_info.user_name" class="content-right">
          {{payment_info.user_name}}
        </span>
        <span v-else class="content-right">--</span>
      </div>
      <div class="content-box">
        <span class="content-label">网银交易流水号</span>
        <span v-if="payment_info && payment_info.obssid" class="content-right">
          {{payment_info.obssid}}
        </span>
        <span v-else class="content-right">--</span>
      </div>
      <div class="content-box">
        <span class="content-label">付款凭证</span>
        <span v-if="payment_info.attachment && payment_info.attachment.length > 0" class="content-rightimg">
          <div v-for="(item, index) in payment_info.attachment" class="pic-wrap" :key='index'>
...
<script>
import {payDetail} from 'src/api/finance'
export default {
  data () {
    return {
      ...
      payDetail({
        ...
        success: (res) => {
          ...
          if (result.code === 0) {
            ...
            this.payment_info = result.data.payment_info
          } else {
            this.$message({
              type: 'error',
              message: result.msg
