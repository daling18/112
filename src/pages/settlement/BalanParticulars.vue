<!--D:\files\repo\bom\application\index\view\dev\src\components\finace\accountBalance\particulars.vue-->
<template>
	<div  class="wrapper">
		<!-- system-search -->
      ...
      <div class="cont-hd">
        <div class="bar clearfix">
          <template v-if="pageSource == 'my'">
            <span class="btn"
              v-if="$menuPermission('settlement/fundsett/accbalan/recharge')"
              @click="recharge(true)">在线充值</span>
            <span class="btn btn2"
              v-if="$menuPermission('settlement/fundsett/accbalan/putforward')"
...
export default {
  ...
  created(){
    if(this.$route.name == 'settlementSettBalanParticularMy'){
      this.pageSource = 'my';
    }else{
  ...
  methods:{
    handleSearch(bool){
      ...
      if(this.pageSource == 'my'){
        this.roleTxt = '我';
        this.roleLinkUrl = 'balan-freeze-my';
      }else{
        this.roleTxt = '他';
        this.roleLinkUrl = 'balan-freeze-other';
      }
      let params = {
