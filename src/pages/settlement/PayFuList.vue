<template>
  <div class="wrapper">
    <div class='system-tab'>
      <div class="right">
        <router-link :to="{path: 'pay-edit',query:{action:1,from:'fu'}}"
          v-if="$menuPermission('settlement/pay/paylist/regpay')">
          <el-button type="primary">+ 登记付款</el-button>
        </router-link>
        <router-link :to="{path: 'boc-edit',query:{}}"
          v-if="$menuPermission('settlement/pay/paylist/onlinetranspay')">
          <el-button type="primary">在线转账付款</el-button>
        </router-link>
      </div>
      <ul class="tab-box">
        <li @click="goto('apply-list',{})" v-if="$menuPermission('settlement/pay/applylist')">付款申请</li>
        <li @click="goto('dispose-list',{})" v-if="$menuPermission('settlement/pay/disposelist')">付款处理</li>
        <li class="on">付款记录</li>
      </ul>
    </div>
