    ...
    <div class="receive-money-wrap clearfix">
      <el-table :data="invoiceDetail" border style="width: 100%">
        ...
        <el-table-column label="应收ID" align="center" width="80">
          <template slot-scope="scope">
            <component :is="$menuPermission('settlement/tuanac/gettuanac/getincodet') ? 'router-link' : 'span'" :to="getIncomeDetailUrl(scope.row)">
              {{ scope.row.bills_id }}
            </component>
          </template>
        </el-table-column>
...
export default {
  ...
  methods: {
    ...
    getOrderDetailUrl(row) {
      return `/settlement/line/order-detail/${row.resource_id}/${row.order_id}`
      // switch(this.from){
      // case 'operator': 
      // case 'sale':
    ...
    getIncomeDetailUrl(row) {
      return `/settlement/tuan-bill/income-detail?bid=${row.bills_id}`;
      // switch(this.from){
      // case 'operator': 
      // case 'sale':
