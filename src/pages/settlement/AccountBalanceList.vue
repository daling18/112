<!--D:\files\repo\bom\application\index\view\dev\src\components\finace\accountBalance\list.vue-->
<template>
	<div  class="wrapper">
            ...
            <template slot-scope="scope">
              <span style="color:#FF6000;">{{scope.row.purchase_amount}}</span>
					...
					<el-table-column label="操作" align="center" width="100"  fixed="right" :resizable="false">
						<template slot-scope="scope">
							<el-dropdown v-if="$menuPermission('settlement/fundsett/accbalan/getbalan') && scope.row.org_id" trigger="click">
								<span class="el-dropdown-link">操作<i class="el-icon-caret-bottom el-icon--right"></i></span>
								<el-dropdown-menu slot="dropdown">
									<el-dropdown-item v-if="$menuPermission('settlement/fundsett/accbalan/getbalan')">
										<router-link :to="{path:'balan-part-my', query: {settlement_org_id:scope.row.org_id}}">
                      余额明细
                    </router-link>
									</el-dropdown-item>
								</el-dropdown-menu>
