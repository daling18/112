<!--D:\files\repo\bom\application\index\view\dev\src\components\plan\applied\appliedPayOnline.vue-->
<template>
  ...
  methods: {
    ...
    handleTransfer () {
      if (this.totalAmount > 0) {
        let tempAmount = this.totalAmount.toString();
        console.log(tempAmount);
        if(tempAmount.split(".")[1] && tempAmount.split(".")[1].length > 2){
          this.$message.error('请款金额最多精确到两位小数，且转账金额必须大于等于现有请款金额')
        }else{
      ...
      bocTransfer({
        ...
        success: (res) => {
          ...
          if (result.code === 0) {
            ...
            // this.$router.back();
            this.$router.push({ path: 'fukuan-detail', query: {id:this.$route.query.id} })
          } else {
            this.$message({
              type: 'error',
              message: result.msg
