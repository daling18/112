<!--D:\files\repo\bom\application\index\view\dev\src\components\finace\balance\dire-settlement.vue-->
<template>
  <div  class="wrapper">
    <div class="system-tab">
      <ul class="tab-box">
        <li v-if="$menuPermission('settlement/fundsett/peer')" @click="goto('peer-list',{})">同行结算</li>
        <li class="on">直客结算</li>
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <template v-if="scope.row.rmb_receipt_amount==scope.row.rmb_receipted_amount">
                <a href="javascript:;"  v-if="$menuPermission('settlement/recpay/shoukuan')"  @click="goto('/settlement/settle/guest-list/receive-record',{id:scope.row.id})">收款记录</a>
                <template v-else>--</template>
              </template>
              <template v-else-if="!scope.row.have_payment">
                <a href="javascript:;" v-if="$menuPermission('settlement/recpay/recei/addrecei')" @click="gain(scope.row.id)" >添加收款</a>
                <template v-else>--</template>
              </template>
              <template v-else>
                <a href="javascript:;" v-if="$menuPermission('settlement/recpay/recei/addrecei')" @click="gain(scope.row.id)">添加收款</a><br>
                <a href="javascript:;" v-if="$menuPermission('settlement/recpay/shoukuan')" @click="goto('/settlement/settle/guest-list/receive-record',{id:scope.row.id})">收款记录</a>
              </template>
            </template>
          </el-table-column>
      ...
      <div class="form-content">
        ...
        <el-form-item label="本次收款金额" required class="mb0">
          <el-form-item prop="total_amount" class="ip1">
            <el-input v-model="ruleForm.total_amount" placeholder="总收款金额" type="number" @blur="ruleTo2()"></el-input>
          </el-form-item>
          <currency-picker
            class="ip2"
