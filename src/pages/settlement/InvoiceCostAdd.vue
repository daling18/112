      ...
      <div class="bd">
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div class="ellipsis" :title="scope.row.tuan_name">
                <span v-if="scope.row.product_id == '' && $menuPermission('settlement/linedetail')">{{scope.row.tuan_no}}&nbsp;&nbsp;{{scope.row.tuan_name}}</span>
                <router-link v-else :to="{path: '/settlement/line/trip-detail', query: {id: scope.row.product_id}}">{{scope.row.tuan_no}}&nbsp;&nbsp;{{scope.row.tuan_name}}</router-link>
              </div>
            </template>
          </el-table-column>
          <el-table-column
          ...
          >
            <template slot-scope="scope">
              <component
                :is="$menuPermission('settlement/tuanac/gettuanac/getcostdet') ? 'router-link' : 'span'"
                :to="`/settlement/tuan-bill/cost-detail?id=${scope.row.bills_id}`"
                target="_blank"
              >
                {{ scope.row.bills_id }}
              </component>
      ...
      <div class="channel-content">
        <el-form label-position="right" label-width="200px" >
          <el-form-item label="成本票金额" required>
            <div class="info">{{ info.amount }} 元</div>
          </el-form-item>
          ...
          <el-form-item label="开户行及账号" style="white-space: nowrap;">
            ...
            <router-link
              class="color-purple"
              v-if="$menuPermission('settlement/conacc/complist')"
              to="/settlement/conacc/comp-list"
              target="_blank">
              公司账户
            </router-link>
          </el-form-item>
          ...
          <el-form-item label="开票方开户行及账户" style="white-space: nowrap;">
            <el-select
            ...
            <router-link class="color-purple"
              ...
              to="/settlement/conacc/peer-list"
              target="_blank">
              同行账户
            </router-link>
          </el-form-item>
          ...
          <el-form-item label="凭证种类" required>
            <el-select placeholder="请选择" v-model="info.voucher_type">
        ...
        <el-table
          ...
          >
            <template slot-scope="scope">
              <component
                :is="$menuPermission('settlement/tuanac/gettuanac/getcostdet') ? 'router-link' : 'span'"
                :to="`/settlement/tuan-bill/cost-detail?id=${scope.row.id}`"
                target="_blank"
              >
                {{ scope.row.id }}
              </component>
...
export default {
  ...
  methods: {
        ...
        success: (res) => {
          ...
          if(request.code === 0) {
            ...
            if(id){
              ...
              if(parentId && info.supply_org_id) {
                const selectBillsOrg = {
                  supplier_org_id: info.supply_org_id, 
                  supplier_org_name: info.supply_org_name
                };
                this.info.supply_org_id = ''
                this.billsOrgList = [selectBillsOrg];
                this.handleSelectBillsOrg(selectBillsOrg)
              } else {
    ...
    getAddInvoice4BillsId() {
      const id = this.$route.query.id || 0;
      
      invoiceAddCostSingle({
        params: {data: { bills_id: id }},
        success: (res) => {// 单项
          if(res.data.code == 0){
            const data = res.data.data;
            this.info = data.invoice_info;
            this.info.supply_org_id = this.info.supplier_org_id;
            // 开票方带入值
            this.selectBillsOrg = {
              supplier_org_id: this.info.supply_org_id, 
              supplier_org_name: this.info.supply_org_name
            };
            this.billsOrgList = [this.selectBillsOrg];
            // 公司账户带入值
            if(this.info.account_id){
              this.selectAccount = {
                id: info.account_id, 
                account_no: info.bank_number, 
                bank_name: info.bank_name
            ...
            // 同行账户带入值
            if(this.info.peer_accout_id){
              this.selectPeerAccount = {
                id: this.info.peer_accout_id, 
                bank_number: this.info.peer_bank_number, 
                bank_name: this.info.peer_bank_name
              };
              this.peerAccountList = [this.selectPeerAccount];
            }else{
              this.getMatchPeerAccount('');
            ...
            this.table = [];
            this.table.push(data.bills_info);
            this.selectBillsOrg = {
              supplier_org_id: this.info.supply_org_id, 
              supplier_org_name: this.info.supply_org_name
            };
