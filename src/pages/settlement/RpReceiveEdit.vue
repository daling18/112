<!--D:\files\repo\bom\application\index\view\dev\src\components\finace\balance\recieve-edit.vue-->
<template>
  <div class="wrapper">
          ...
          <el-form :model="ruleForm" :rules="rules" ref="ruleForm" label-width="170px" class="demo-ruleForm">
            ...
            <el-form-item label="本次收款金额（本位币）">
             ...
             <el-form-item label="收款方式／账号" prop="account_id" required style="white-space: nowrap;">
              ...
              <a href="javascript:;" class="color-purple" v-if="isSale || isFromTuancontro" @click="handleAddAccount">添加账户</a>
    ...
    <el-dialog title="选择团队收入项" :visible.sync="dialogTableVisible" width="100%">
      <div class="system-search">
        <div class="search-line-item">
          <div class="search-item" v-if="!isFromTuancontro">
      ...
      <div class="search-content">
        <el-table class="el-table--middle" :data="choose_tuan" border>
          ...
          <el-table-column property="name" label="结算方" header-align="center" align="left">
            <template slot-scope="scope">
              {{scope.row.distributor_org_name}}
            </template>
          </el-table-column>
          <el-table-column property="name" label="项目名称" header-align="center" align="left">
            <template slot-scope="scope">
              {{scope.row.item_name}}
            </template>
          </el-table-column>
          <el-table-column property="name" label="备注" header-align="center" align="left">
            <template slot-scope="scope">
              {{scope.row.remark}}
            </template>
          </el-table-column>
...
export default {
  ...
  computed: {
    ...
    isFromTuancontro(){
      // 来源 团控中心/线路团控/收入管理的添加收款
      // 等于以前的判断isPlan
      if(this.$route.query.from == 'tuancontro'){
        return true;
      }
    },
    isFromSettlement(){
  ...
  mounted() {
    ...
    if(this.isFromTuancontro){
      ...
      if(this.$route.name == 'settlementRecpayReceiveEdit'){
        // 收款认款里面的添加登记收款
        const data = {name: ''};
  ...
  methods: {
    operateSuccessRedirect(){
      ...
      setTimeout(() => {
        ...
        //   }
        // }
        this.$router.push({ path: '/settlement/recpay/shou-list', query: {} })
      }, 500);
    },
      ...
      let action = this.$route.query.action,
        ...
        params = {
          data: {
            ...
            tuan_id: this.isFromTuancontro ? this.tuanId : ''
    ...
    submitForm(formName) {
      this.$refs[formName].validate((valid) => {
        if (valid) {
          ...
          if((this.isSale || this.isFromTuancontro) && tuan.length == 0){
