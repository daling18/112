          ...
          >
            ...
            header-align="center"
					...
					<el-table-column label="操作" align="center" width="100"  fixed="right" :resizable="false">
						<template slot-scope="scope">
              <template v-if="!isShowOperate(0,scope)">--</template>
							<el-dropdown v-else trigger="click">
								<span class="el-dropdown-link">操作<i class="el-icon-caret-bottom el-icon--right"></i></span>
								<el-dropdown-menu slot="dropdown">
                  <el-dropdown-item v-if="isShowOperate(1,scope)">
                    <a href="javascript:;" @click="editConfigure(scope)">
                      授信配置
                    </a>
                  </el-dropdown-item>
									<el-dropdown-item v-if="isShowOperate(2,scope)">
										<router-link :to="{path:'/settlement/settle/peer-list/receive-record', query: {id:scope.row.settlement_org_id}}">收款记录</router-link>
									</el-dropdown-item>
									<el-dropdown-item v-if="isShowOperate(3,scope)">
										<router-link :to="{path: '/settlement/settle/balan-part-other', query: {settlement_org_id:scope.row.settlement_org_id}}">余额明细</router-link>
									</el-dropdown-item>
									<el-dropdown-item v-if="isShowOperate(4,scope)">
										<router-link :to="{path: '/settlement/settle/peer-list/pay-record', query: {id:scope.row.settlement_org_id}}">付款记录</router-link>
									</el-dropdown-item>
									<el-dropdown-item v-if="isShowOperate(5,scope)">
										<router-link :to="{path: '/settlement/invoice/income/list', query: {id:scope.row.settlement_org_id}}">收入发票</router-link>
									</el-dropdown-item>
									<el-dropdown-item v-if="isShowOperate(6,scope)">
										<router-link :to="{path: '/settlement/invoice/cost/list', query: {id:scope.row.settlement_org_id}}">成本回票</router-link>
									</el-dropdown-item>
								</el-dropdown-menu>
							</el-dropdown>
						</template>
...
export default {
  ...
  methods:{
      ...
      isShowOperate(edit, data) {
        if (!edit) {
          return [1,2,3,4,5,6].some(e => this.isShowOperate(e, data));
        }
        switch (edit) {
        case 1: 
          // 授信配置
          return this.$menuPermission('settlement/fundsett/peer/greditconf');
        case 2: 
          // 收款记录
          return this.$menuPermission('settlement/recpay/shoukuan');
        case 3: 
          // 余额明细
          return this.$menuPermission('settlement/fundsett/peer/getbalance');
        case 4: 
          // 付款记录
          return this.$menuPermission('settlement/pay/fukuan');
        case 5: 
          // 收入发票
          return this.$menuPermission('settlement/invoice/income');
        case 6: 
          // 成本回票
          return this.$menuPermission('settlement/invoice/cost');
        default:
          return false;
        }
      },
