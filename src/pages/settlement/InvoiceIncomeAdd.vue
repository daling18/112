<template>
  <div class="cont">
    <div class="panel" v-if="type == 1">
      ...
      <div class="bd">
        <el-table
          ...
          <el-table-column
            ...
            <template slot-scope="scope">
              <div class="ellipsis" :title="scope.row.tuan_name">
                ...
                <router-link v-else :to="{path: '/settlement/line/trip-detail', query: {id: scope.row.product_id}}" target="_blank">{{scope.row.tuan_no}}&nbsp;&nbsp;{{scope.row.tuan_name}}</router-link>
              </div>
            </template>
          </el-table-column>
          <el-table-column
            ...
            <template slot-scope="scope">
              <component
                ...
                :is="$menuPermission('settlement/getorderdetail') ? 'router-link' : 'span'"
                :to="`/settlement/line/order-detail/${scope.row.resource_id}/${scope.row.order_id}`"
                target="_blank"
              >
                {{ scope.row.order_no }}
              </component>
          ...
          <el-table-column label="应收ID" align="center" width="80">
            <template slot-scope="scope">
              <component
                :is="$menuPermission('settlement/tuanac/gettuanac/getincodet') ? 'router-link' : 'span'"
                :to="`/settlement/tuan-bill/income-detail?bid=${scope.row.bills_id}`"
                target="_blank"
              >
                {{ scope.row.bills_id }}
              </component>
        ...
        <el-table
          ...
          <el-table-column label="应收ID" align="center" width="80">
            <template slot-scope="scope">
              <component
                :is="$menuPermission('settlement/tuanac/gettuanac/getincodet') ? 'router-link' : 'span'"
                :to="`/settlement/tuan-bill/income-detail?bid=${scope.row.id}`"
                target="_blank"
              >
                {{ scope.row.id }}
              </component>
    ...
    <div class="btn-wrap">
      <el-button size="large" type="primary" @click="handleAddIncome">保存</el-button>
        ...
        success: (res) => {
          ...
          if(request.code === 0) {
            ...
            if(id){
              ...
              if(parentId && info.raised_org_id) {
                const selectBillsOrg = {
                  taxpayer_num: info.taxpayer_num, 
                  distributor_org_id: info.raised_org_id, 
                  distributor_org_name: info.raised_org_name
                };
                this.info.raised_org_id = ''
