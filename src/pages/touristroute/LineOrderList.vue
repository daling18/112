<template>
<!-- application\index\view\dev\src\components\plan\OrderList.vue -->
  <div class="wrapper">
    <!-- system-search -->
        ...
        <div class="list">
          <el-table class="el-table--middle" :data="tableData" border style="width: 100%">
            ...
            <el-table-column label="游客" header-align="center" width="100" :resizable="false">
              <template slot-scope="scope">
                ...
                <router-link v-if="scope.row.is_tourist_cancel == 1"
                  :to="{ path: '/touristroute/order-list/order-refund', query: {ono: scope.row.order_no, tid: scope.row.tuan_id}}">
                  [退]
            ...
            <el-table-column label="操作" width="70" align="center" fixed="right">
              <template slot-scope="scope">
                <router-link v-if="$menuPermission('touristroute/lineorder/lineorder/getorderdetail')" :to="'/touristroute/order-list/order-detail/' + scope.row.tuan_id + '/' + scope.row.id">
                  查看
                </router-link>
                <div v-else>--</div>
              </template>
    ...
    <div class="page" v-show="pageShow">
      <el-pagination layout="total, prev, pager, next, jumper" :total="totalCount" :current-page="searchData.page" @current-change="handleCurrentChange" :page-count="totalPage"/>
    </div>
  </div>
</template>
