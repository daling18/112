...
<!-- application\index\view\dev\src\components\plan\trip\trip-list.vue -->
  <div class="wrapper trip-wrpper">
    <div class="system-tab">
      <ul class="tab-box">
        <router-link :class='name === "trip" ? "on" : ""' to="/touristroute/other-list" v-if="$menuPermission('touristroute/linetravel/company') && (ORG_TYPE !== '6')" tag="li">本公司行程</router-link>
          ...
          <el-table :data="res_list" class="el-table--middle" border style="width: 100%">
            ...
            <el-table-column
              ...
              <template slot-scope="scope">
                ...
                <div
                  v-else-if="name === 'other'"
                  class="ellipsis  pointer" style="text-align: center;">
            ...
            <el-table-column
             ...
             >
              <template slot-scope="scope">
                <div v-if='name === "supplierTrip"'>
                  <el-dropdown v-if="($menuPermission('usercenter/approval/detail') || $menuPermission('touristroute/linetravel/supplier/getstrockdetail'))" trigger="click">
                    ...
                    <el-dropdown-menu slot="dropdown">
                      ...
                      <el-dropdown-item v-if="$menuPermission('touristroute/linetravel/supplier/getstrockdetail') && scope.row.supplier_audit_status === 2">
                        <a href="javascript:;" @click="soldoutProduct(scope.row.id,[scope.row.org_id] )"
                        >下架</a>
                      </el-dropdown-item>
                    </el-dropdown-menu>
                  </el-dropdown>
                ...
                <div v-if='!showBtn(0, scope) && name === "trip"'>--</div>
                <el-dropdown v-else-if='name === "trip"' trigger="click">
                  <span class="el-dropdown-link">
                    操作
                    <i class="el-icon-caret-bottom el-icon--right"></i>
                  </span>
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            ...
            <template v-if="channelType == 1">
              ...
              >上架</a>
              <el-button type='text' @click="handleProductOperation(scope, 'putaway')" v-else-if="scope.row.submit_status == 6">重新上架</el-button>
            </template>
            <template v-else-if="channelType == 3">
              <span class="link-col" @click="yeySync(scope.row.product_id)">同步</span>
            </template>
            <!-- <template v-if="channelType === 5 || channelType === 0">
...
<script>
  ...
  import {handleNewPutOn, 
          ...
          otherProductList,
          getChannelList}from 'src/api/product'
    ...
    methods: {
      ...
      handleAllSelectDownWay(){
        if(this.allSelectData){
          ...
          this.soldoutProduct(this.middleProduct.row.id,arr)
        }else{
          this.$message({
            message: "请选择下架渠道！",
            type: 'error'
      ...
      handleAllSelectPutWay(){
        if(this.allSelectData){
          ...
          this.putawayProduct(this.middleProduct.row.id,arr)
        }else{
          this.$message({
            message: "请选择上架渠道！",
            type: 'error'
      ...
      putawayProduct(product_id,org_ids){
        this.$confirm(
          '确认上架该行程？',
          '上架',
          {type: 'warning'}
      ...
      soldoutProduct(product_id,org_ids){
        
        let action = handleNewSelfDown;
        if(this.name === 'other'){
          action = handleNewOtherDown
        }else if (this.name === 'supplierTrip'){
          action = handleNewSupplierDown
        }
      ...
      handleProductOperation(scope, oper){
        ...
        if(oper == 'show'){
          ...
          this.putawayProduct(data.product_id,[data.org_id]);
        }else if(oper == 'soldout'){
          this.soldoutProduct(data.product_id,[data.org_id]);
        }else{
