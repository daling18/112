// src\components\system\approval\tripDetail.vue
<template>
  <!-- wrapper -->
  <div class="wrapper" >
    <!-- system-tab -->
    ...
    <div class="system-content">
      ...
      <template v-else-if="approvalType == 11">
        <InvoiceIncomeDetailTemp v-if="detail.type == 1" :invoice-info="detail.invoice_info" :invoice-detail="detail.invoice_detail" :log-info="detail.log_info" />
        <InvoiceCostDetailTemp v-else :invoice-info="detail.invoice_info" :invoice-detail="detail.invoice_detail" :log-info="detail.log_info" />
      </template>
      ...
      <template v-else-if="approvalType == 17">
        <div class="type17">
          ...
          <div class="detail-section">
            <div class="detail-tit">办理流程</div>
            <div class="process">
              <i class="l"></i>
              <div class="process-item-custom" v-for="(step, idx) in info.visa_process" :key="idx">
                <b><i>{{ idx + 1 }}</i></b>
                <div class="process-name-custom">{{ step.title }}</div>
                <div class="process-desc-custom">{{ step.content }}</div>
              </div>
            </div>
          </div>
          <div class="detail-section">
              ...
              <div class="detail-con">
                <div class="popover-table">
                    <table>
                      <tr>
                        <th>退票规则</th>
                        <td>{{ scenicDetail.refund_rule_arr.refund_rule }}</td>
                      </tr>
                      <tr>
                        <th>是否需要审核</th>
                        <td>{{ scenicDetail.refund_rule_arr.refund_audit }}</td>
                      </tr>
                      <tr>
                        <th>退票手续费</th>
                        <td>
                          {{ scenicDetail.refund_rule_arr.refund_rule == '不可退' ? 0 : scenicDetail.refund_rule_arr.refund_fee }}
                        </td>
                      </tr>
                      <tr v-if="scenicDetail.refund_rule_arr.refund_rule != '不可退'">
                        <th>阶梯退票</th>
                        <td>
                          <p v-for="(item,idx) in scenicDetail.refund_rule_arr.refund_cost" :key="idx">
                            {{item}}
                          </p>
                        </td>
                      </tr>
      ...
      <template v-else-if="approvalType == 23">
        <div class="type20">
          <div class="content">
            <div class='bd'>
              <table v-for="(obj,idx) in airTicket" :key="idx" class="table-order">
                <thead>
                  <tr>
                    ...
                    <th width="90">订单损失金额</th>
                    <th>退票类型</th>
                    <th width="80">退款状态</th>
                    <th width="90">实际退款金额</th>
                    <th width="90">财务退款状态</th>
                    <th width="80">操作</th>
                  </tr>
                </thead>
                <tbody>
                  ...
                  <td class="tc"><strong class="color-orange">&yen;{{airTicketPrice.refund_amount}}</strong></td>
                  <td class="tc"><strong class="color-orange">&yen;{{airTicketPrice.refund_fee}}</strong></td>
                  <td>{{obj.refund_type_name}}</td>
                  <td>{{obj.refund_status_name}}</td>
                  <td>{{obj.real_refund_amount}}</td>
                  <td>{{obj.finance_refund_status ==1 ? '未处理':'已处理'}}</td>
                  <td class="tc">
                    ---
                  </td>
                </tr>
                <tr>
                  <td colspan="10">
                    <table>
                      <tr>
                        <th width="80">申请内容</th>
                        <td v-if="obj.refund_pic_url">
                          <!-- <p><span class="tit">【退票说明】</span>{{obj.refund_type_name}}</p>
                          <p><span class="tit">【申退原因】</span>{{obj.refund_reason}}</p> -->
                          <div v-for="(tmp,index) in obj.refund_pic_url" :key="index" class="refund-img">
                            <img :src="tmp" alt="" class="tag-img">
                          </div>
                        </td>
                      </tr>
                      <tr>
                        <th width="80">失败原因</th>
                        <td v-if="obj.remark">{{obj.remark}}</td>
                      </tr>
                    </table>
                  </td>
                </tr>
                <tr>
                  <td colspan="10">
                    <p v-for="(approval, approvalKey) in obj.refund_approval" :key="approvalKey">
                      <span class="label">[定损人/时间]</span>
...
import CalendarQuotation from 'src/pages/singlepro/components/CalendarQuotation.vue';
import AttachmentList from 'src/plugins/AttachmentList.vue'
import InvoiceIncomeDetailTemp from 'src/pages/settlement/components/InvoiceIncomeDetailTemp.vue'
import InvoiceCostDetailTemp from 'src/pages/settlement/components/InvoiceCostDetailTemp.vue'
export default {
  filters: {
      ...
      approval_content({
        ...
        success: res => {
          ...
          if (code == 0) {
            ...
            }else if(this.approvalType == 11){
              data.info.type = data.type; // 发票类型： 1. 收入发票； 2. 成本发票
            ...
            }else if(this.approvalType == 23){
              this.airTicket[0] = data.refund;
              this.airTicketPrice = data.params;
            }else{
