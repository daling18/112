<template>
  <div class="wrapper">
    <div class="system-tab">
      <ul class="tab-box">
        <router-link
          tag="li"
          :class="loadType == 0 ? 'on' : ''"
          v-if="$menuPermission('usercenter/approval/myinitappr')"
          to="../submit/list"
        >我发起的({{myinitappr}})</router-link>
        <router-link
          tag="li"
          :class="loadType == 1 ? 'on' : ''"
          v-if="$menuPermission('usercenter/approval/waitmyapproval')"
          to="../check/list"
        >待我审批({{waitmyapproval}})</router-link>
        <router-link
          tag="li"
          :class="loadType == 2 ? 'on' : ''"
          v-if="$menuPermission('usercenter/approval/myapprovaled')"
          to="../checked/list"
        >我已审批({{myapprovaled}})</router-link>
      </ul>
    </div>
    <!-- system-search -->
    <div class="system-search">
      <div class="search-item">
        <div class="label">审批类型</div>
        <el-select v-model="searchData.approval_type" placeholder="请选择">
          <el-option label="全部" value />
          <el-option v-for="(o,i) in allData.approval_types" :key="i" :label="o" :value="i" />
        </el-select>
      </div>
      <div class="search-item">
        <div class="label">审批状态</div>
        <el-select v-model="searchData.status" placeholder="请选择">
          <template v-if="loadType == 1">
            <el-option label="审批中" value="1" />
          </template>
          <template v-else>
            <el-option label="全部" value />
            <el-option label="审批中" value="1" />
            <el-option label="审批通过" value="2" />
            <el-option label="审批不通过" value="3" />
            <el-option label="已撤销" value="4" />
          </template>
        </el-select>
      </div>
      <div class="search-item">
        <div class="label">审批编号</div>
        <el-input v-model="searchData.approval_no" placeholder="审批单编号" />
      </div>
      <div class="search-item">
        <div class="label">审批标题</div>
        <el-input v-model="searchData.title" placeholder="审批单标题" />
      </div>
      <search-item-date-range v-model="calendarDateCreate" label="发起日期" />
      <search-item-date-range v-model="calendarDateComplete" label="完成日期" />
      <div class="search-item search-button">
        <el-button type="primary" @click="handleSearch(true)">立即搜索</el-button>
      </div>
    </div>
    <!-- /system-search -->
    <div class="system-content">
      <div class="content-list">
        <el-table :data="tableData" style="width: 100%" border>
          <el-table-column
            prop="approval_no"
            label="审批单编号"
            header-align="center"
            width="160"
            :resizable="false"
          />
          <el-table-column
            prop="title"
            label="审批单标题"
            header-align="center"
            width="150"
            :resizable="false"
          />
          <el-table-column
            prop="approval_type_name"
            label="审批类型"
            header-align="center"
            width="150"
            :resizable="false"
          />
          <el-table-column label="发起人" header-align="center" min-width="220" :resizable="false">
            <template slot-scope="scope">
              <div class="fqr">
                <p>
                  <b>发起人：</b>
                  {{ scope.row.initiator_name }}
                </p>
                <el-tooltip
                  class="item"
                  effect="dark"
                  :content="scope.row.org_code_name"
                  placement="top"
                >
                  <p>
                    <b>所属组织：</b>
                    <span class="ellipsis lin">{{ scope.row.org_code_name }}</span>
                  </p>
                </el-tooltip>
                <!-- <p><b>部门：</b>{{ scope.row.initiator_dep_name }}</p>
                <p><b>公司：</b>{{ scope.row.company_name }}</p>-->
              </div>
            </template>
          </el-table-column>
          <el-table-column
            prop="create_time"
            label="发起时间"
            align="center"
            width="100"
            :resizable="false"
          />
          <el-table-column
            prop="complete_time"
            label="完成时间"
            align="center"
            width="100"
            :resizable="false"
          />
          <el-table-column
            prop="status_name"
            label="审批状态"
            align="center"
            width="100"
            :resizable="false"
          />
          <el-table-column label="操作" width="80" align="center" fixed="right" :resizable="false">
            <template slot-scope="scope">
              <el-link
                class="alink curpoint"
                @click="handleSponsorSp(scope.row)"
                v-if="loadType == 1 && $menuPermission('usercenter/approval/myinitappr/apppass')"
              >审批</el-link>
              <template v-if="scope.row.approval_type == 1">
                <el-link
                  v-if="detailPerm"
                  @click="$router.push({
                 path:`/touristroute/trip/approval-detail/${scope.row.id}`
               })"
                >审批单</el-link>
              </template>
              <el-link
                v-else-if="detailPerm"
                @click="$router.push({path:'detail', query:{id:scope.row.id}})"
              >审批单</el-link>
              <span
                v-else-if="!(loadType == 1 && $menuPermission('usercenter/approval/myinitappr/apppass'))"
              >--</span>
            </template>
          </el-table-column>
        </el-table>
      </div>
    </div>
    <div class="page">
      <el-pagination
        layout="prev, pager, next"
        v-if="pagination.totalPage > 0"
        @current-change="handleCurrentChange"
        :current-page="pagination.currentPage"
        :total="pagination.totalCount"
        :page-size="pagination.size"
      />
    </div>
    <!--审批弹窗-->
    <el-dialog
      title="审批操作"
      :visible.sync="diaSpVisible"
      @close="cleanForm('formSpRef')"
      size="tiny"
    >
      <el-form :model="formSp" ref="formSpRef">
        <el-form-item label="审批" prop="operation">
          <el-radio-group v-model="formSp.operation">
            <el-radio label="1">同意</el-radio>
            <el-radio label="2">不同意</el-radio>
          </el-radio-group>
        </el-form-item>
        <el-form-item class="upload-wrapper" label="审批附件">
          <el-upload
            ref="notifications_upload"
            action="/bom/api/product/upload_pic"
            :data="{type: 1}"
            :show-file-list="false"
            :file-list="fileList"
            :on-success="handleSuccess"
            :before-upload="handleBeforeUpload"
          >
            <el-button type="default">上传附件</el-button>
            <span>最多上传3张，支持jpg，jpeg，gif，png格式，10M以内图片</span>
          </el-upload>
        </el-form-item>
        <el-form-item>
          <div class="pic-box">
            <div v-for="(item,index) in imgArr" :key="index" class="pic-item">
              <span class="el-icon-close" @click="handleRemoveImg(index)"></span>
              <div class="item">
                <img :src="item" alt />
              </div>
            </div>
          </div>
        </el-form-item>
        <el-form-item label="备注" prop="remark">
          <el-input type="textarea" v-model="formSp.remark" />
        </el-form-item>
      </el-form>
      <span slot="footer" class="dialog-footer">
        <el-button @click="handleSponsorSp(null,'cancel')">取 消</el-button>
        <el-button @click="handleSponsorSp(null,'submit')" type="primary">确 定</el-button>
      </span>
    </el-dialog>
    <!--/审批弹窗-->
  </div>
</template>

<script>
import {
  my_start_approval_list,
  my_wait_approval_list,
  already_approval_list,
  deal_approval,
  deal_approval_new
} from "src/api/approval";
import { formattDate } from "src/assets/js/common.js";
import SearchItemDateRange from "src/plugins/SearchItemDateRange";
export default {
  components: {
    SearchItemDateRange
  },
  data() {
    return {
      imgArr: [],
      fileList: [],
      loadType: null,
      searchDataOrigin: {
        approval_type: "",
        approval_no: "",
        title: "",
        status: "",
        create_time_start: "",
        create_time_end: "",
        complete_time_start: "",
        complete_time_end: "",
        page: this.$route.query.page || 1
      },
      searchData: {
        approval_type: "",
        approval_no: "",
        title: "",
        status: "",
        create_time_start: "",
        create_time_end: "",
        complete_time_start: "",
        complete_time_end: "",
        page: 1
      },
      calendarDateCreate: ["", ""],
      calendarDateComplete: ["", ""],
      allData: [],
      tableData: [],
      pagination: {
        currentPage: 1,
        totalCount: 0,
        totalPage: 0,
        size: 10
      },
      pageShow: false,
      // 审批弹窗属性
      diaSpVisible: false,
      formSp: {
        approval_id: "",
        operation: "1",
        remark: ""
      },
      myinitappr: null,
      waitmyapproval: 20,
      myapprovaled: 58,
      IsSupplier: false
    };
  },
  computed: {
    detailPerm() {
      return this.$menuPermission("usercenter/approval/detail");
      // switch(this.loadType) {
      // case 0:
      //   return this.$menuPermission('usercenter/approval/myinitappr/getappdet')
      // case 1:
      //   return this.$menuPermission('usercenter/approval/waitmyapproval/getappdet')
      // case 2:
      //   return this.$menuPermission('usercenter/approval/myapprovaled/getappdet')
      // default:
      //   return false;
      // }
    }
  },
  watch: {
    $route(to, from) {
      if (to.name != from.name) {
        this.searchDataOrigin.approval_type = "";
        this.searchData.approval_no = "";
        this.searchDataOrigin.status = "";
        this.searchDataOrigin.title = "";
        this.searchDataOrigin.create_time_start = "";
        this.searchDataOrigin.create_time_end = "";
        this.searchDataOrigin.complete_time_start = "";
        this.searchDataOrigin.complete_time_end = "";
        switch (to.name) {
          case "approvalListsubmit":
            this.loadType = 0;
            this.searchData.status = "";
            break;
          case "approvalListcheck":
            this.loadType = 1;
            this.searchData.status = "1";
            break;
          case "approvalListchecked":
            this.loadType = 2;
            this.searchData.status = "";
            break;
        }
        this.handleSearch(true);
      }
    }
  },
  created: function() {
    switch (this.$route.name) {
      case "approvalListsubmit":
        this.loadType = 0;
        break;
      case "approvalListcheck":
        this.loadType = 1;
        break;
      case "approvalListchecked":
        this.loadType = 2;
        break;
    }
  },
  methods: {
    handleRemoveImg(index) {
      this.fileList.splice(index, 1);
      this.imgArr.splice(index, 1);
    },
    // handleSuccess(response, file, fileList) {
    //   if (response.code === 0) {
    //     this.imgArr.push(response.data.path);
    //   } else {
    //     this.$message({
    //       type: "error",
    //       message: response.msg
    //     });
    //   }
    // },
    // 图片上传-删除图片
    // handleRemove(file) {
    //     for (let i = 0; i < this.notificationsForm.file_info.length; i++) {
    //         if (this.notificationsForm.file_info[i]["id"] == file.id) {
    //             this.notificationsForm.file_info.splice(i, 1);
    //             return false;
    //         }
    //     }
    // },
    // 图片上传-图片更新成功
    handleSuccess(response, file) {
        if (response.code == 0) {
            this.fileList.push(response.data)
            this.imgArr.push(response.data.path);
            // this.$set(this, "fileList", response.data);
        } else {
            this.$message.error(response.msg);
        }
    },
    // 图片上传-图片更新前
    handleBeforeUpload(file) {
       if (this.fileList.length >= 3) {
         this.$message.error('最多上传三张图片');
         return false
       }
        let isLt2M = file.size / 1024 / 1024 < 10;
        let isTrue = /jpeg|png|jpg|pdf/i.test(
            file.name.split(".")[file.name.split(".").length - 1]
        );
        if (!isTrue) {
            this.$message.error("仅支持jpeg、png、jpg、pdf格式!");
            return false;
        }
        if (!isLt2M) {
            this.$message.error("上传文件大小不能超过 10MB!");
            return false;
        }
        return true;
    },
    handleSearch(bool) {
      if (bool) {
        this.searchData.page = 1;
        this.searchData.create_time_start = this.calendarDateCreate[0];
        this.searchData.create_time_end = this.calendarDateCreate[1];
        this.searchData.complete_time_start = this.calendarDateComplete[0];
        this.searchData.complete_time_end = this.calendarDateComplete[1];
        this.searchDataOrigin = { ...this.searchData };
      } else {
        this.searchData = { ...this.searchDataOrigin };
        this.calendarDateCreate = [
          this.searchData.create_time_start,
          this.searchData.create_time_end
        ];
        this.calendarDateComplete = [
          this.searchData.complete_time_start,
          this.searchData.complete_time_end
        ];
      }
      this.$router.replace({ query: this.searchData });
      let params = {
        data: this.searchData
      };
      let api = "";
      switch (this.loadType) {
        case 0:
          api = my_start_approval_list;
          break;
        case 1:
          api = my_wait_approval_list;
          break;
        case 2:
          api = already_approval_list;
          break;
      }
      api({
        params,
        success: res => {
          let { code, data, msg } = res.data;
          if (code == 0) {
            this.allData = data;
            this.tableData = data.data;
            this.pagination.currentPage = data.page_index;
            this.pagination.totalCount = data.total_count;
            this.pagination.totalPage = data.total_page;
            this.pageShow = data.total_page > 1 ? true : false;
            this.myinitappr = data.getCount.my_approve;
            this.waitmyapproval = data.getCount.wait_approve;
            this.myapprovaled = data.getCount.already_approve;
          } else {
            this.$message.error(msg);
          }
        },
        showLoading: true
      });
    },
    handleCurrentChange(val) {
      this.searchDataOrigin.page = val;
      this.handleSearch();
    },
    async handleSponsorSp(row, type) {
      if (row != null) {
        this.diaSpVisible = true;
        this.formSp.approval_id = row.id;
        row.approval_type == "7" || row.approval_type == "8"
          ? (this.IsSupplier = true)
          : "";
      } else {
        if (type == "submit") {
          let para = {};
          let arrImg = "";
          let imgArr = this.imgArr;
          if (imgArr.length >= 0) {
            for (let i = 0; i < imgArr.length; i++) {
              if (i < imgArr.length - 1) {
                arrImg += imgArr[i] + ",";
              } else {
                arrImg += imgArr[i];
              }
            }
          }

          para.data = {
            operation: this.formSp.operation,
            approval_id: this.formSp.approval_id,
            remark: this.formSp.remark,
            attachment: arrImg
          };
          this.diaSpVisible = false;
          if (this.IsSupplier) {
            const params = {
              approval_id: this.formSp.approval_id,
              operation: this.formSp.operation,
              remark: this.formSp.remark,
              attachment: arrImg
            };
            const { code, data, msg } = await deal_approval_new(params);
            if (code !== 0) return this.$message.error(msg);

            this.$message.success(msg);
            this.imgArr = [];
            this.handleSearch();
          } else {
            deal_approval({
              params: para,
              success: res => {
                let { code, data, msg } = res.data;
                if (code == 0) {
                  this.$message.success(msg);
                  this.handleSearch();
                } else {
                  this.$message.error(msg);
                }
              },
              showLoading: true
            });
            this.imgArr = [];
          }
        }
        this.diaSpVisible = false;
      }
    },
    cleanForm(refName) {
      this.$refs[refName].resetFields();
    }
  },
  mounted() {
    this.handleSearch();
  }
};
</script>

<style scoped lang="less">
.el-badge.badge {
  margin-top: 10px;
  margin-right: 40px;
}

.system-header {
  margin-bottom: 10px;
  padding: 15px;
  background: #fff;
}
.page {
  position: relative;
  background: #fff;
  padding: 20px 0;
  height: 30px;
}
.fqr {
  .lin {
    line-height: 18px;
  }
  line-height: 18px;
  padding-left: 75px;
  b {
    float: left;
    width: 75px;
    line-height: 18px;
    margin-left: -75px;
    font-weight: 400;
    text-align: right;
  }
}
.alink {
  color: #b10c79;
  &:hover {
    opacity: 0.8;
  }
}
.pic-box {
  overflow: hidden;
  padding: 10px;
  .pic-item {
    width: 100px;
    height: 100px;
    margin: 10px;
    float: left;
    position: relative;
    background: #ccc;
    span {
      position: absolute;
      right: -10px;
      top: -10px;
      font-size: 16px;
    }
    .item {
      width: 100px;
      height: 100px;
      overflow: hidden;
    }
    img {
      display: block;
      width: 100%;
      height: auto;
    }
  }
}
</style>
