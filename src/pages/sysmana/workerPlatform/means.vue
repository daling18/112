    ...
    <div id="worker-container">
        <div class="worker-page" v-if="pageIndex == 0">
            <div class="worker-header">
                ...
                <div v-if="!permission" style="text-align:center;font-size:30px;">宣传资料</div>
            </div>
            <div class="worker-body">
                <el-table @cell-click="showDetail" width="100%" :data="tableData" v-loading="loading" border>
                    <el-table-column label="标题" prop="title" min-width="200" max-width='300' align="left">
                    </el-table-column>
                    ...
                    <el-table-column v-if='permission' label="管理操作" min-width="160" align="center">
        ...
        <!--编辑添加-->
        <addEdit v-if="pageIndex == 1" ref="addEdit" :type="2" @switchPage="switchPage"></addEdit>
        <detail v-if="pageIndex == 2" :type="2" @switchPage="switchPage"></detail>
    </div>
</template>
...
export default {
    ...
    mounted() {
        this.permission = this.$menuPermission("workplatform/means/permission");
        if (this.params.id) {
            this.switchPage(2);
        } else {
            this.getList(true);
