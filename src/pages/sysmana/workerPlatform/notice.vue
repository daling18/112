<template>
    <div id="worker-container">
        ...
        <!--编辑添加-->
        <addEdit v-if="pageIndex == 1" ref="addEdit" :type="1" @switchPage="switchPage"></addEdit>

        <detail v-if="pageIndex == 2" :type="1" @switchPage="switchPage"></detail>
    </div>
</template>
...
export default {
    ...
    mounted() {
        this.permission = this.$menuPermission(
            "workplatform/notice/permission"
        );
        if (this.params.id) {
            this.switchPage(2);
        } else {
