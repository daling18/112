// src\components\system\approval\listConfig.vue
<template>
  <div class="wrapper" >
    <div class="system-tab">
      ...
      <router-link class="right-btn"
        v-if="$menuPermission('sysmana/appconf/aeappconf')"
        :to="{path:'gain', query:{id:'',ty:'ga'}}">
        新增审批配置
      </router-link>
    </div>
    <!-- system-search -->
      ...
      <div class="content-list">
        <el-table :data="tableData" style="width: 100%" border>
          ...
          <el-table-column label="操作" width="150" align="center" :resizable="false">
            <template slot-scope="scope">
              <template v-if="$menuPermission('sysmana/appconf/aeappconf')">
                <router-link :to="{path:'gain', query:{id:scope.row.id,ty:'ed'}}">
                ...
                <router-link v-if="$menuPermission('sysmana/appconf/copyappconf')" :to="{path:'gain', query:{id:scope.row.id,ty:'co'}}">
                  复制
                </router-link>
                <span
                  class="alink"
                  @click="affirmDelete(scope.row.id)"
                  v-if="scope.row.have_ing_flag != 1 && $menuPermission('sysmana/appconf/delappconf')">
                  删除
                </span>
              </template>
